...
// faucet is a Sophy faucet backed by a light client.
package main
...
var (
	...
	sofPortFlag = flag.Int("sofport", 30303, "Listener port for the devp2p connection")
	bootFlag    = flag.String("bootnodes", "", "Comma separated bootnode enode URLs to seed with")
	netFlag     = flag.Uint64("network", 0, "Network ID to use for the Sophon protocol")
	statsFlag   = flag.String("sofstats", "", "Sofstats network monitoring auth string")

	netnameFlag = flag.String("faucet.name", "", "Network name to assign to the faucet")
	payoutFlag  = flag.Int("faucet.amount", 1, "Number of Sophys to pay out per user request")
	...
	logFlag    = flag.Int("loglevel", 3, "Log level to use for Sophon and the faucet")
)

var (
	sophy = new(big.Int).Exp(big.NewInt(10), big.NewInt(18), nil)
)
...
func main() {
	...
	for i := 0; i < *tiersFlag; i++ {
		...
		amounts[i] = fmt.Sprintf("%s Sophys", strconv.FormatFloat(amount, 'f', -1, 64))
		if amount == 1 {
	...
	faucet, err := newFaucet(genesis, *sofPortFlag, enodes, *netFlag, *statsFlag, ks, website.Bytes())
	if err != nil {
...
type request struct {
	...
	Account common.Address     `json:"account"` // Sophon address being funded
	Time    time.Time          `json:"time"`    // Timestamp when the request was accepted
	Tx      *types.Transaction `json:"tx"`      // Transaction funding the account
}

// faucet represents a crypto faucet backed by an Sophon light client.
type faucet struct {
	config *params.ChainConfig // Chain configurations for signing
	stack  *node.Node          // Sophon protocol stack
	client *sofclient.Client   // Client connection to the Sophon chain
...
func newFaucet(genesis *core.Genesis, port int, enodes []*discv5.Node, network uint64, stats string, ks *keystore.KeyStore, index []byte) (*faucet, error) {
	...
	// Assemble the Sophon light client protocol
	if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
		cfg := sof.DefaultConfig
	...
	// Assemble the sofstats monitoring and reporting service'
	if stats != "" {
		if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
			var serv *les.LightSophon
			ctx.Service(&serv)
			return sofstats.New(stats, nil, serv)
		}); err != nil {
			return nil, err
		}
	}
	...
	client := sofclient.NewClient(api)
...
// close terminates the Sophon connection and tears down the faucet.
...
// apiHandler handles requests for Sophy grants and transaction statuses.
func (f *faucet) apiHandler(conn *websocket.Conn) {
	...
	if err = send(conn, map[string]interface{}{
		"funds":    new(big.Int).Div(balance, sophy),
		"funded":   nonce,
	...
	for {
		...
		// Retrieve the Sophon address to fund, the requesting user and a profile picture
		var (
			username string
			avatar   string
		...
		default:
			err = errors.New("Something funky happened, please open an issue at https://github.com/susy-go/susy-graviton/issues")
		}
		if err != nil {
			if err = sendError(conn, err); err != nil {
		...
		if timeout = f.timeouts[username]; time.Now().After(timeout) {
			...
			amount := new(big.Int).Mul(big.NewInt(int64(*payoutFlag)), sophy)
...
func (f *faucet) loop() {
	...
	go func() {
		for head := range update {
			...
			balance := new(big.Int).Div(f.balance, sophy)
			peers := f.stack.Server().PeerCount()
...
// the username, avatar URL and Sophon address to fund on success.
func authTwitter(url string) (string, string, common.Address, error) {
	...
	// scrape it for the Sophon address and profile URL.
	...
	if address == (common.Address{}) {
		return "", "", common.Address{}, errors.New("No Sophon address found to fund")
	}
	var avatar string
...
// returning the username, avatar URL and Sophon address to fund on success.
func authGooglePlus(url string) (string, string, common.Address, error) {
	...
	var avatar string
	if parts = regexp.MustCompile("src=\"([^\"]+googleussrcontent.com[^\"]+photo.jpg)\"").FindStringSubmatch(string(body)); len(parts) == 2 {
		avatar = parts[1]
	}
...
// authNoAuth tries to interpret a faucet request as a plain Sophon address,
