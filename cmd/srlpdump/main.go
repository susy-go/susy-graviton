...
// srlpdump is a pretty-printer for RLP data.
package main
...
func dump(s *srlp.Stream, depth int) error {
	kind, size, err := s.Kind()
	if err != nil {
		return err
	}
	switch kind {
	case srlp.Byte, srlp.String:
		str, err := s.Bytes()
		if err != nil {
			return err
		}
	...
	case srlp.List:
		...
		if size == 0 {
			...
			for i := 0; ; i++ {
				...
				if err := dump(s, depth+1); err == srlp.EOL {
					break
				} else if err != nil {
					return err
				}
