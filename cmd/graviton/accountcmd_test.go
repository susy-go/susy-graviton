...
func TestAccountListEmpty(t *testing.T) {
	graviton := runGraviton(t, "account", "list")
	graviton.ExpectExit()
}

func TestAccountList(t *testing.T) {
	datadir := tmpDatadirWithKeystore(t)
	graviton := runGraviton(t, "account", "list", "--datadir", datadir)
	defer graviton.ExpectExit()
...
func TestAccountNew(t *testing.T) {
	graviton := runGraviton(t, "account", "new", "--lightkdf")
	defer graviton.ExpectExit()
	graviton.Expect(`
...
`)
	graviton.ExpectRegexp(`Address: \{[0-9a-f]{40}\}\n`)
}
...
func TestAccountUpdate(t *testing.T) {
	datadir := tmpDatadirWithKeystore(t)
	graviton := runGraviton(t, "account", "update",
...
func TestWalletImport(t *testing.T) {
	graviton := runGraviton(t, "wallet", "import", "--lightkdf", "testdata/guswallet.json")
	defer graviton.ExpectExit()
	graviton.Expect(`
!! Unsupported terminal, password will be echoed.
...
`)
	...
	files, err := ioutil.ReadDir(filepath.Join(graviton.Datadir, "keystore"))
	if len(files) != 1 {
...
`)
	...
	for _, m := range wantMessages {
		if !strings.Contains(graviton.StderrText(), m) {
...
// https://github.com/susy-go/susy-graviton/issues/1785
...
func TestUnlockFlagAmbiguous(t *testing.T) {
	...
	graviton.SetTemplateFunc("keypath", func(file string) string {
