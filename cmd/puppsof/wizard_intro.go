...
// makeWizard creates and returns a new puppsof wizard.
...
// setting up a new or managing an existing Sophon private network.
func (w *wizard) run() {
	fmt.Println("+-----------------------------------------------------------+")
	fmt.Println("| Welcome to puppsof, your Sophon private network manager |")
	fmt.Println("|                                                           |")
	fmt.Println("| This tool lets you create a new Sophon network down to  |")
	fmt.Println("| the genesis block, bootnodes, miners and sofstats servers |")
	fmt.Println("| without the hassle that it would normally entail.         |")
	fmt.Println("|                                                           |")
	fmt.Println("| Puppsof uses SSH to dial in to remote servers, and builds |")
	...
	log.Info("Administering Sophon network", "name", w.network)

	// Load initial configurations and connect to all live servers
	w.conf.path = filepath.Join(os.Getenv("HOME"), ".puppsof", w.network)
