...
FROM puppsof/wallet:latest

ADD genesis.json /genesis.json

RUN \
  echo 'node server.js &'                     > wallet.sh && \
	echo 'graviton --cache 512 init /genesis.json' >> wallet.sh && \
	echo $'exec graviton --networkid {{.NetworkID}} --port {{.NodePort}} --bootnodes {{.Bootnodes}} --sofstats \'{{.Sofstats}}\' --cache=512 --rpc --rpcaddr=0.0.0.0 --rpccorsdomain "*" --rpcvhosts "*"' >> wallet.sh

RUN \
	sed -i 's/PuppsofNetworkID/{{.NetworkID}}/g' dist/js/sophywallet-master.js && \
	sed -i 's/PuppsofNetwork/{{.Network}}/g'     dist/js/sophywallet-master.js && \
	sed -i 's/PuppsofDenom/{{.Denom}}/g'         dist/js/sophywallet-master.js && \
	sed -i 's/PuppsofHost/{{.Host}}/g'           dist/js/sophywallet-master.js && \
	sed -i 's/PuppsofRPCPort/{{.RPCPort}}/g'     dist/js/sophywallet-master.js
