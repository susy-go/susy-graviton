...
func (w *wizard) deployNode(boot bool) {
	...
	if w.conf.sofstats == "" {
		log.Error("No sofstats server configured")
		return
	}
	...
	if w.conf.Genesis.Config.Sofash != nil && !boot {
		fmt.Println()
		if infos.sofashdir == "" {
			fmt.Printf("Where should the sofash mining DAGs be stored on the remote machine?\n")
			infos.sofashdir = w.readString()
		} else {
			fmt.Printf("Where should the sofash mining DAGs be stored on the remote machine? (default = %s)\n", infos.sofashdir)
			infos.sofashdir = w.readDefaultString(infos.sofashdir)
		}
	}
	...
	if infos.sofstats == "" {
		...
		infos.sofstats = w.readString() + ":" + w.conf.sofstats
	} else {
		fmt.Printf("What should the node be called on the stats page? (default = %s)\n", infos.sofstats)
		infos.sofstats = w.readDefaultString(infos.sofstats) + ":" + w.conf.sofstats
	}
	// If the node is a miner/signer, load up needed credentials
	if !boot {
		if w.conf.Genesis.Config.Sofash != nil {
			// Sofash based miners only need an sophybase to mine against
			fmt.Println()
			if infos.sophybase == "" {
				fmt.Printf("What address should the miner use?\n")
				for {
					if address := w.readAddress(); address != nil {
						infos.sophybase = address.Hex()
						break
					}
				}
			} else {
				fmt.Printf("What address should the miner use? (default = %s)\n", infos.sophybase)
				infos.sophybase = w.readDefaultAddress(common.HexToAddress(infos.sophybase)).Hex()
			}
	...
	if out, err := deployNode(client, w.network, w.conf.bootnodes, infos, nocache); err != nil {
		log.Error("Failed to deploy Sophon node container", "err", err)
		if len(out) > 0 {
			fmt.Printf("%s\n", out)
		}
		return
