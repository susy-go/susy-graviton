...
// nodeDockerfile is the Dockerfile required to run an Sophon node.
var nodeDockerfile = `
FROM sophon/client-go:latest
...
{{end}}
RUN \
  echo 'graviton --cache 512 init /genesis.json' > graviton.sh && \{{if .Unlock}}
	echo 'mkdir -p /root/.sophon/keystore/ && cp /signer.json /root/.sophon/keystore/' >> graviton.sh && \{{end}}
	echo $'exec graviton --networkid {{.NetworkID}} --cache 512 --port {{.Port}} --nat extip:{{.IP}} --maxpeers {{.Peers}} {{.LightFlag}} --sofstats \'{{.Sofstats}}\' {{if .Bootnodes}}--bootnodes {{.Bootnodes}}{{end}} {{if .Sophybase}}--miner.sophybase {{.Sophybase}} --mine --miner.threads 1{{end}} {{if .Unlock}}--unlock 0 --password /signer.pass --mine{{end}} --miner.gastarget {{.GasTarget}} --miner.gaslimit {{.GasLimit}} --miner.gasprice {{.GasPrice}}' >> graviton.sh

ENTRYPOINT ["/bin/sh", "graviton.sh"]
`

// nodeComposefile is the docker-compose.yml file required to deploy and maintain
// an Sophon node (bootnode or miner for now).
var nodeComposefile = `
version: '2'
services:
  {{.Type}}:
    ...
    ports:
      ...
      - "{{.Port}}:{{.Port}}/udp"
    volumes:
      - {{.Datadir}}:/root/.sophon{{if .Sofashdir}}
      - {{.Sofashdir}}:/root/.sofash{{end}}
    environment:
      - PORT={{.Port}}/tcp
      - TOTAL_PEERS={{.TotalPeers}}
      - LIGHT_PEERS={{.LightPeers}}
      - STATS_NAME={{.Sofstats}}
      - MINER_NAME={{.Sophybase}}
...
// deployNode deploys a new Sophon node container to a remote machine via SSH,
// docker and docker-compose. If an instance with the specified network name
// already exists there, it will be overwritten!
func deployNode(client *sshClient, network string, bootnodes []string, config *nodeInfos, nocache bool) ([]byte, error) {
	kind := "sealnode"
	if config.keyJSON == "" && config.sophybase == "" {
		kind = "bootnode"
	...
	template.Must(template.New("").Parse(nodeDockerfile)).Execute(dockerfile, map[string]interface{}{
		...
		"Sofstats":  config.sofstats,
		"Sophybase": config.sophybase,
...
func (info *nodeInfos) Report() map[string]string {
	...
	if info.gasTarget > 0 {
		...
		if info.sophybase != "" {
			// Sofash proof-of-work miner
			report["Sofash directory"] = info.sofashdir
			report["Miner account"] = info.sophybase
		}
		if info.keyJSON != "" {
...
func checkNode(client *sshClient, network string, boot bool) (*nodeInfos, error) {
	...
	var out []byte
	if out, err = client.Run(fmt.Sprintf("docker exec %s_%s_1 graviton --exec admin.nodeInfo.enode --cache=16 attach", network, kind)); err != nil {
	...
	stats := &nodeInfos{
		genesis:    genesis,
		datadir:    infos.volumes["/root/.sophon"],
		sofashdir:  infos.volumes["/root/.sofash"],
		port:       port,
		peersTotal: totalPeers,
		peersLight: lightPeers,
		sofstats:   infos.envvars["STATS_NAME"],
		sophybase:  infos.envvars["MINER_NAME"],
		keyJSON:    keyJSON,
		keyPass:    keyPass,
		gasTarget:  gasTarget,
		gasLimit:   gasLimit,
