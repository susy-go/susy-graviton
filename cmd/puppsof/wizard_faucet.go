...
func (w *wizard) deployFaucet() {
	...
	fmt.Println()
	fmt.Printf("How many Sophys to release per request? (default = %d)\n", infos.amount)
	...
	fmt.Println()
	if infos.node.sofstats == "" {
		fmt.Printf("What should the node be called on the stats page?\n")
		infos.node.sofstats = w.readString() + ":" + w.conf.sofstats
	} else {
		fmt.Printf("What should the node be called on the stats page? (default = %s)\n", infos.node.sofstats)
		infos.node.sofstats = w.readDefaultString(infos.node.sofstats) + ":" + w.conf.sofstats
	}
