...
func (w *wizard) deployDashboard() {
	...
	for _, service := range []string{"sofstats", "explorer", "wallet", "faucet"} {
		...
		for _, server := range available[service] {
			...
			switch service {
			case "sofstats":
				if infos, err := checkSofstats(client, w.network); err == nil {
					port = infos.port
				}
			case "explorer":
	...
	// If we have sofstats running, ask whether to make the secret public or not
	if w.conf.sofstats != "" {
		fmt.Println()
		fmt.Println("Include sofstats secret on dashboard (y/n)? (default = yes)")
