...
// config contains all the configurations needed by puppsof that should be saved
// between sessions.
type config struct {
	path      string   // File containing the configuration values
	bootnodes []string // Bootnodes to always connect to by all nodes
	sofstats  string   // Sofstats settings to cache for node deploys
...
func (c config) flush() {
	...
	if err := ioutil.WriteFile(c.path, out, 0644); err != nil {
		log.Warn("Failed to save puppsof configs", "file", c.path, "err", err)
	}
}
...
type wizard struct {
	...
	services map[string][]string   // Sophon services known to be running on servers
...
// converts it to an Sophon address. If an empty line is entered, the default
// value is returned.
...
// weird formats used by sofstats, which compares IPs textually, not by value.
