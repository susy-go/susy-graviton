...
func (w *wizard) gatherStats(server string, pubkey []byte, client *sshClient) *serverStat {
	...
	logger.Debug("Checking for sofstats availability")
	if infos, err := checkSofstats(client, w.network); err != nil {
		if err != ErrServiceUnknown {
			stat.services["sofstats"] = map[string]string{"offline": err.Error()}
		}
	} else {
		stat.services["sofstats"] = infos.Report()
		sofstats = infos.config
	}
