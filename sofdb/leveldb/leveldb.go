...
func (db *Database) NewBatch() sofdb.Batch {
	return &batch{
		db: db.db,
		b:  new(leveldb.Batch),
	}
...
func (db *Database) NewIterator() sofdb.Iterator {
...
// not exist).
func (db *Database) NewIteratorWithStart(start []byte) sofdb.Iterator {
	return db.db.NewIterator(&util.Range{Start: start}, nil)
}

// NewIteratorWithPrefix creates a binary-alphabetical iterator over a subset
// of database content with a particular key prefix.
func (db *Database) NewIteratorWithPrefix(prefix []byte) sofdb.Iterator {
...
func (b *batch) Replay(w sofdb.KeyValueWriter) error {
	return b.b.Replay(&replayer{writer: w})
}

// replayer is a small wrapper to implement the correct replay methods.
type replayer struct {
	writer  sofdb.KeyValueWriter
	failure error
}
