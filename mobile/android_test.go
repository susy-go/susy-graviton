...
import (
	...
	"github.com/susy-go/susy-graviton/internal/build"
)
...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testAccountManagement() {
		...
		KeyStore ks = new KeyStore(getInstrumentation().getContext().getFilesDir() + "/keystore", Graviton.LightScryptN, Graviton.LightScryptP);
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.sophon", new NodeConfig());
			node.start();
			...
			SophonClient ec = node.getSophonClient();
	...
	// Tests that recovering transaction signers works for both Homestead and SIP155
	// signatures too. Regression test for susy-graviton issue #14599.
	public void testIssue14599() {
		try {
			byte[] preSIP155RLP = new BigInteger("", 16).toByteArray();
			preSIP155RLP = Arrays.copyOfRange(preSIP155RLP, 1, preSIP155RLP.length);

			byte[] postSIP155RLP = new BigInteger("f86b80847735940082520894ef5bbb9bba2e1ca69ef81b23a8727d889f3ef0a1880de0b6b3a7640000802ba06fef16c44726a102e6d55a651740636ef8aec6df3ebf009e7b0c1f29e4ac114aa057e7fbc69760b522a78bb568cfc37a58bfdcf6ea86cb8f9b550263f58074b9cc", 16).toByteArray();
			postSIP155RLP = Arrays.copyOfRange(postSIP155RLP, 1, postSIP155RLP.length);

			Transaction preSIP155 =  new Transaction(preSIP155RLP);
			Transaction postSIP155 = new Transaction(postSIP155RLP);

			preSIP155.getFrom(null);           // Homestead should accept homestead
			preSIP155.getFrom(new BigInt(4));  // SIP155 should accept homestead (missing chain ID)
			postSIP155.getFrom(new BigInt(4)); // SIP155 should accept SIP 155

			try {
				postSIP155.getFrom(null);
				fail("SIP155 transaction accepted by Homestead");
			} catch (Exception e) {}
		} catch (Exception e) {
			fail(e.toString());
		}
...
func TestAndroid(t *testing.T) {
	// Skip tests on Windows altogsophy
	...
	workspace, err := ioutil.TempDir("", "graviton-android-")
	if err != nil {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/sophon/gravitontest", "libs"} {
		err = os.MkdirAll(dir, os.ModePerm)
		if err != nil {
			t.Fatal(err)
		}
	}
	// Generate the mobile bindings for Graviton and add the tester class
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.sophon", "github.com/susy-go/susy-graviton/mobile")
	if output, err := gobind.CombinedOutput(); err != nil {
		t.Logf("%s", output)
		t.Fatalf("failed to run gomobile bind: %v", err)
	}
	build.CopyFile(filepath.Join("libs", "graviton.aar"), "graviton.aar", os.ModePerm)

	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "org", "sophon", "gravitontest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
...
<manifest xmlns:android="http://schemas.android.com/apk/res/android"
          package="org.sophon.gravitontest"
