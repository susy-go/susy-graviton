...
func TestNodeIterator(t *testing.T) {
	var (
		fulldb  = sofdb.NewMemDatabase()
		lightdb = sofdb.NewMemDatabase()
		gspec   = core.Genesis{Alloc: core.GenesisAlloc{testBankAddress: {Balance: testBankFunds}}}
		genesis = gspec.MustCommit(fulldb)
	)
	gspec.MustCommit(lightdb)
	blockchain, _ := core.NewBlockChain(fulldb, nil, params.TestChainConfig, sofash.NewFullFaker(), vm.Config{}, nil)
	gchain, _ := core.GenerateChain(params.TestChainConfig, genesis, sofash.NewFaker(), fulldb, 4, testChainGen)
