...
func GetChtRoot(db sofdb.Database, sectionIdx uint64, sectionHead common.Hash) common.Hash {
...
// Note that sectionIdx is specified according to LES/1 CHT section size.
func StoreChtRoot(db sofdb.Database, sectionIdx uint64, sectionHead, root common.Hash) {
	var encNumber [8]byte
	binary.BigEndian.PutUint64(encNumber[:], sectionIdx)
...
type ChtIndexerBackend struct {
	diskdb, trieTable    sofdb.Database
	odr                  OdrBackend
...
func NewChtIndexer(db sofdb.Database, odr OdrBackend, size, confirms uint64) *core.ChainIndexer {
	trieTable := sofdb.NewTable(db, ChtTablePrefix)
	...
	return core.NewChainIndexer(db, sofdb.NewTable(db, "chtIndex-"), backend, size, confirms, time.Millisecond*100, "cht")
}
...
func (c *ChtIndexerBackend) Process(ctx context.Context, header *types.Header) error {
	...
	data, _ := srlp.EncodeToBytes(ChtNode{hash, td})
...
func GetBloomTrieRoot(db sofdb.Database, sectionIdx uint64, sectionHead common.Hash) common.Hash {
	var encNumber [8]byte
	binary.BigEndian.PutUint64(encNumber[:], sectionIdx)
...
func StoreBloomTrieRoot(db sofdb.Database, sectionIdx uint64, sectionHead, root common.Hash) {
	var encNumber [8]byte
	binary.BigEndian.PutUint64(encNumber[:], sectionIdx)
...
func NewBloomTrieIndexer(db sofdb.Database, odr OdrBackend, parentSize, size uint64) *core.ChainIndexer {
	trieTable := sofdb.NewTable(db, BloomTrieTablePrefix)
	...
	return core.NewChainIndexer(db, sofdb.NewTable(db, "bltIndex-"), backend, size, 0, time.Millisecond*100, "bloomtrie")
}
