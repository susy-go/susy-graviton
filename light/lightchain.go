...
// available in the database. It initialises the default Sophon header
// validator.
...
func (lc *LightChain) GetBodyRLP(ctx context.Context, hash common.Hash) (srlp.RawValue, error) {
	// Short circuit if the body's already in the cache, retrieve otherwise
	if cached, ok := lc.bodyRLPCache.Get(hash); ok {
		return cached.(srlp.RawValue), nil
	}
	number := lc.hc.GetBlockNumber(hash)
	if number == nil {
		return nil, errors.New("unknown block")
