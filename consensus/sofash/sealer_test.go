...
func TestRemoteNotify(t *testing.T) {
	...
	// Create the custom sofash engine
	sofash := NewTester([]string{"http://" + listener.Addr().String()}, false)
	defer sofash.Close()
	...
	sofash.Seal(nil, block, nil, nil)
	select {
	case work := <-sink:
		if want := sofash.SealHash(header).Hex(); work[0] != want {
...
func TestStaleSubmission(t *testing.T) {
	...
	for id, c := range testcases {
		for _, h := range c.headers {
			sofash.Seal(nil, types.NewBlockWithHeader(h), results, nil)
		}
		if res := api.SubmitWork(fakeNonce, sofash.SealHash(c.headers[c.submitIndex]), fakeDigest); res != c.submitRes {
		...
		case <-time.NewTimer(time.Second).C:
			t.Errorf("case %d fetch sofash result timeout", id+1)
		}
	}
}
