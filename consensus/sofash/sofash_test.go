...
// Tests that sofash works correctly in test mode.
func TestTestMode(t *testing.T) {
	header := &types.Header{Number: big.NewInt(1), Difficulty: big.NewInt(100)}

	sofash := NewTester(nil, false)
	defer sofash.Close()

	results := make(chan *types.Block)
	err := sofash.Seal(nil, types.NewBlockWithHeader(header), results, nil)
	if err != nil {
	...
	case block := <-results:
		...
		if err := sofash.VerifySeal(nil, header); err != nil {
...
// It reproduces https://github.com/susy-go/susy-graviton/issues/14943
func TestCacheFileEvict(t *testing.T) {
	tmpdir, err := ioutil.TempDir("", "sofash-test")
	if err != nil {
		t.Fatal(err)
	}
...
func verifyTest(wg *sync.WaitGroup, e *Sofash, workerIndex, epochs int) {
	defer wg.Done()
...
func TestRemoteSealer(t *testing.T) {
	...
	sealhash := sofash.SealHash(header)

	// Push new work.
	results := make(chan *types.Block)
	sofash.Seal(nil, block, results, nil)
...
func TestHashRate(t *testing.T) {
	...
	if tot := sofash.Hashrate(); tot != float64(expect) {
...
func TestClosedRemoteSealer(t *testing.T) {
	...
	api := &API{sofash}
	if _, err := api.GetWork(); err != errSofashStopped {
		t.Error("expect to return an error to indicate sofash is stopped")
	}

	if res := api.SubmitHashRate(hexutil.Uint64(100), common.HexToHash("a")); res {
		t.Error("expect to return false when submit hashrate to a stopped sofash")
	}
}
