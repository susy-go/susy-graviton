...
const (
	// staleThreshold is the maximum depth of the acceptable stale but valid sofash solution.
	staleThreshold = 7
)
...
func (sofash *Sofash) Seal(chain consensus.ChainReader, block *types.Block, results chan<- *types.Block, stop <-chan struct{}) error {
	...
	if sofash.config.PowMode == ModeFake || sofash.config.PowMode == ModeFullFake {
		...
		default:
			log.Warn("Sealing result is not read by miner", "mode", "fake", "sealhash", sofash.SealHash(block.Header()))
		}
		return nil
	}
	// If we're running a shared PoW, delegate sealing to it
	if sofash.shared != nil {
		return sofash.shared.Seal(chain, block, results, stop)
	}
	// Create a runner and the multiple search threads it directs
	abort := make(chan struct{})

	sofash.lock.Lock()
	threads := sofash.threads
	if sofash.rand == nil {
		...
		sofash.rand = rand.New(rand.NewSource(seed.Int64()))
	}
	sofash.lock.Unlock()
	if threads == 0 {
	...
	if sofash.workCh != nil {
		sofash.workCh <- &sealTask{block: block, results: results}
	}
	var (
		pend   sync.WaitGroup
	...
	for i := 0; i < threads; i++ {
		...
		go func(id int, nonce uint64) {
			defer pend.Done()
			sofash.mine(block, id, nonce, abort, locals)
		}(i, uint64(sofash.rand.Int63()))
	}
	...
	go func() {
		...
		case result = <-locals:
			...
			case results <- result:
			default:
				log.Warn("Sealing result is not read by miner", "mode", "local", "sealhash", sofash.SealHash(block.Header()))
			}
			close(abort)
		case <-sofash.update:
			// Thread count was changed on user request, restart
			close(abort)
			if err := sofash.Seal(chain, block, results, stop); err != nil {
...
func (sofash *Sofash) mine(block *types.Block, id int, seed uint64, abort chan struct{}, found chan *types.Block) {
	// Extract some data from the header
	var (
		header  = block.Header()
		hash    = sofash.SealHash(header).Bytes()
		target  = new(big.Int).Div(two256, header.Difficulty)
		number  = header.Number.Uint64()
		dataset = sofash.dataset(number, false)
	)
	...
	logger.Trace("Started sofash search for new nonces", "seed", seed)
search:
	for {
		select {
		case <-abort:
			// Mining terminated, update stats and abort
			logger.Trace("Sofash nonce search aborted", "attempts", nonce-seed)
			sofash.hashrate.Mark(attempts)
			break search
		...
		default:
			...
			if new(big.Int).SetBytes(result).Cmp(target) <= 0 {
				...
				case found <- block.WithSeal(header):
					logger.Trace("Sofash nonce found and reported", "attempts", nonce-seed, "nonce", nonce)
				case <-abort:
					logger.Trace("Sofash nonce found but discarded", "attempts", nonce-seed, "nonce", nonce)
				}
				break search
			}
			nonce++
...
func (sofash *Sofash) remote(notify []string, noverify bool) {
	var (
	...
	makeWork := func(block *types.Block) {
		hash := sofash.SealHash(block.Header())
	...
	submitWork := func(nonce types.BlockNonce, mixDigest common.Hash, sealhash common.Hash) bool {
		...
		start := time.Now()
		if !noverify {
			if err := sofash.verifySeal(nil, header, true); err != nil {
		...
		if results == nil {
			log.Warn("Sofash result channel is empty, submitted mining result is rejected")
			return false
		}
	...
	for {
		...
		case result := <-sofash.submitWorkCh:
		...
		case errc := <-sofash.exitCh:
			// Exit remote loop if sofash is closed and return relevant error.
			errc <- nil
			log.Trace("Sofash remote sealer is exiting")
			return
		}
	}
}
