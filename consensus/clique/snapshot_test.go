...
// mapped from textual names used in the tests below to actual Sophon private
...
// address retrieves the Sophon address of a tester account by label, creating
...
func (ap *testerAccountPool) address(account string) common.Address {
	...
	// Resolve and return the Sophon address
...
func TestClique(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			// batch (https://github.com/susy-go/susy-graviton/issues/17593). Whilst this
	...
	for i, tt := range tests {
		...
		for j, block := range blocks {
			// Graviton the header and prepare it for signing
			header := block.Header()
			if j > 0 {
