...
static duk_ret_t duk__handle_require(duk_context *ctx) {
	 ...
	 *  require cache before rsofrowing.  This allows the application to
...
void duk_module_node_init(duk_context *ctx) {
	...
	duk_require_object_cosrcible(ctx, -1);  /* error before setting up requireCache */
