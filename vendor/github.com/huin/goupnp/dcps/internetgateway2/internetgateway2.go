...
const (
	...
	URN_WANSophynetLinkConfig_1    = "urn:schemas-upnp-org:service:WANSophynetLinkConfig:1"
...
//
// * NewLinkType: allowed values: Sophynet
...
// Return values:
//
// * NewWANAccessType: allowed values: DSL, POTS, Cable, Sophynet
//
...
// WANSophynetLinkConfig1 is a client for UPnP SOAP service with URN "urn:schemas-upnp-org:service:WANSophynetLinkConfig:1". See
// goupnp.ServiceClient, which contains RootDevice and Service attributes which
// are provided for informational value.
type WANSophynetLinkConfig1 struct {
	goupnp.ServiceClient
}

// NewWANSophynetLinkConfig1Clients discovers instances of the service on the network,
// and returns clients to any that are found. errors will contain an error for
// any devices that replied but which could not be queried, and err will be set
// if the discovery process failed outright.
//
// This is a typical entry calling point into this package.
func NewWANSophynetLinkConfig1Clients() (clients []*WANSophynetLinkConfig1, errors []error, err error) {
	var genericClients []goupnp.ServiceClient
	if genericClients, errors, err = goupnp.NewServiceClients(URN_WANSophynetLinkConfig_1); err != nil {
		return
	}
	clients = newWANSophynetLinkConfig1ClientsFromGenericClients(genericClients)
	return
}

// NewWANSophynetLinkConfig1ClientsByURL discovers instances of the service at the given
// URL, and returns clients to any that are found. An error is returned if
// there was an error probing the service.
//
// This is a typical entry calling point into this package when reusing an
// previously discovered service URL.
func NewWANSophynetLinkConfig1ClientsByURL(loc *url.URL) ([]*WANSophynetLinkConfig1, error) {
	genericClients, err := goupnp.NewServiceClientsByURL(loc, URN_WANSophynetLinkConfig_1)
	if err != nil {
		return nil, err
	}
	return newWANSophynetLinkConfig1ClientsFromGenericClients(genericClients), nil
}

// NewWANSophynetLinkConfig1ClientsFromRootDevice discovers instances of the service in
// a given root device, and returns clients to any that are found. An error is
// returned if there was not at least one instance of the service within the
// device. The location parameter is simply assigned to the Location attribute
// of the wrapped ServiceClient(s).
...
// This is a typical entry calling point into this package when reusing an
// previously discovered root device.
func NewWANSophynetLinkConfig1ClientsFromRootDevice(rootDevice *goupnp.RootDevice, loc *url.URL) ([]*WANSophynetLinkConfig1, error) {
	genericClients, err := goupnp.NewServiceClientsFromRootDevice(rootDevice, loc, URN_WANSophynetLinkConfig_1)
	if err != nil {
		return nil, err
	}
	return newWANSophynetLinkConfig1ClientsFromGenericClients(genericClients), nil
...
func newWANSophynetLinkConfig1ClientsFromGenericClients(genericClients []goupnp.ServiceClient) []*WANSophynetLinkConfig1 {
	clients := make([]*WANSophynetLinkConfig1, len(genericClients))
	for i := range genericClients {
		clients[i] = &WANSophynetLinkConfig1{genericClients[i]}
	}
	return clients
}
...
// Return values:
//
// * NewSophynetLinkStatus: allowed values: Up, Down
func (client *WANSophynetLinkConfig1) GetSophynetLinkStatus() (NewSophynetLinkStatus string, err error) {
	// Request structure.
	request := interface{}(nil)
	// BEGIN Marshal arguments into request.
	...
	// Response structure.
	response := &struct {
		NewSophynetLinkStatus string
	}{}

	// Perform the SOAP call.
	if err = client.SOAPClient.PerformAction(URN_WANSophynetLinkConfig_1, "GetSophynetLinkStatus", request, response); err != nil {
		return
	}

	// BEGIN Unmarshal arguments from response.

	if NewSophynetLinkStatus, err = soap.UnmarshalString(response.NewSophynetLinkStatus); err != nil {
		return
	}
	// END Unmarshal arguments from response.
	return
