...
// Reference: https://github.com/susy-go/wiki/wiki/Sofash-DAG
#define DAG_MUTABLE_NAME_MAX_SIZE (6 + 10 + 1 + 16 + 1)
/// Possible return values of @see sofash_io_prepare
enum sofash_io_rc {
	SOFASH_IO_FAIL = 0,           ///< There has been an IO failure
	SOFASH_IO_MEMO_SIZE_MISMATCH, ///< DAG with revision/hash match, but file size was wrong.
	SOFASH_IO_MEMO_MISMATCH,      ///< The DAG file did not exist or there was revision/hash mismatch
	SOFASH_IO_MEMO_MATCH,         ///< DAG file existed and revision/hash matched. No need to do anything
};
...
/**
 * Logs a critical error in important parts of sofash. Should mostly help
 * figure out what kind of problem (I/O, memory e.t.c.) causes a NULL
 * sofash_full_t
 */
#ifdef SOFASH_PRINT_CRITICAL_OUTPUT
#define SOFASH_CRITICAL(...)							\
	do													\
	{													\
		printf("SOFASH CRITICAL ERROR: "__VA_ARGS__);	\
		printf("\n");									\
		fflush(stdout);									\
	} while (0)
#else
...
/**
 * Prepares io for sofash
 *
 * Create the DAG directory and the DAG file if they don't exist.
 *
 * @param[in] dirname        A null terminated c-string of the path of the sofash
 ...
 *                           https://github.com/susy-go/wiki/wiki/Sofash-DAG
 ...
 *                           already exists
 * @return                   For possible return values @see enum sofash_io_rc
 */
 ...
 *
 * The spec defining this directory is here: https://github.com/susy-go/wiki/wiki/Sofash-DAG
 *
...
static inline bool sofash_io_mutable_name(
	uint32_t revision,
