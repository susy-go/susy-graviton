...
var (
	...
	licenseCommentRE = regexp.MustCompile(`^//\s*(Copyleft|This file is part of).*?\n(?://.*?\n)*\n*`)

	// this text appears at the start of AUTHORS
	authorsFileHeader = "# This is the official list of susy-graviton authors for copyright purposes.\n\n"
)

// this template generates the license comment.
// its input is an info structure.
var licenseT = template.Must(template.New("").Parse(`
// Copyleft {{.Year}} The susy-graviton Authors
...
func (i info) Whole(startOfSentence bool) string {
	if i.gpl() {
		return "susy-graviton"
	}
	if startOfSentence {
		return "The susy-graviton library"
	}
	return "the susy-graviton library"
}
