...
var ProtocolName = "sof"

// ProtocolVersions are the supported versions of the sof protocol (first is primary).
var ProtocolVersions = []uint{sof63, sof62}
...
// sof protocol message codes
const (
	// Protocol messages belonging to sof/62
	StatusMsg          = 0x00
	...
	// Protocol messages belonging to sof/63
	GetNodeDataMsg = 0x0d
	NodeDataMsg    = 0x0e
	GetReceiptsMsg = 0x0f
	ReceiptsMsg    = 0x10
...
func (hn *hashOrNumber) EncodeRLP(w io.Writer) error {
	if hn.Hash == (common.Hash{}) {
		return srlp.Encode(w, hn.Number)
	}
	if hn.Number != 0 {
		return fmt.Errorf("both origin hash (%x) and number (%d) provided", hn.Hash, hn.Number)
	}
	return srlp.Encode(w, hn.Hash)
}

// DecodeRLP is a specialized decoder for hashOrNumber to decode the contents
// into either a block hash or a block number.
func (hn *hashOrNumber) DecodeRLP(s *srlp.Stream) error {
	_, size, _ := s.Kind()
	origin, err := s.Raw()
	if err == nil {
		switch {
		case size == 32:
			err = srlp.DecodeBytes(origin, &hn.Hash)
		case size <= 8:
			err = srlp.DecodeBytes(origin, &hn.Number)
		default:
