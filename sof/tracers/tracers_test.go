...
import (
	...
	"github.com/susy-go/susy-graviton/tests"
)

// To generate a new callTracer test, copy paste the makeTest method below into
// a Graviton console and call it with a transaction hash you which to export.
...
var makeTest = function(tx, rewind) {
  ...
  var block   = sof.getBlock(sof.getTransaction(tx).blockHash);
  var genesis = sof.getBlock(block.parentHash);
  ...
  genesis.config = admin.nodeInfo.protocols.sof.config;
  ...
  console.log(JSON.stringify({
    ...
    input:  sof.getRawTransaction(tx),
    result: result,
  }, null, 2));
}
*/
...
func TestPrestateTracerCreate2(t *testing.T) {
	...
	signer := types.NewSIP155Signer(big.NewInt(1))
	tx, err := types.SignTx(unsigned_tx, signer, privateKeyECDSA)
	if err != nil {
		t.Fatalf("err %v", err)
	}
	/**
		This comes from one of the test-vectors on the Skinny Create2 - SIP
	...
	*/
	...
	statedb := tests.MakePreState(sofdb.NewMemDatabase(), alloc)
	// Create the tracer, the SVM environment and run it
	tracer, err := New("prestateTracer")
	if err != nil {
		t.Fatalf("failed to create call tracer: %v", err)
	}
	svm := vm.NewSVM(context, statedb, params.MainnetChainConfig, vm.Config{Debug: true, Tracer: tracer})

	msg, err := tx.AsMessage(signer)
	if err != nil {
		t.Fatalf("failed to prepare transaction for tracing: %v", err)
	}
	st := core.NewStateTransition(svm, msg, new(core.GasPool).AddGas(tx.Gas()))
...
func TestCallTracer(t *testing.T) {
	...
	for _, file := range files {
		...
		t.Run(camel(strings.TrimSuffix(strings.TrimPrefix(file.Name(), "call_tracer_"), ".json")), func(t *testing.T) {
			...
			if err := srlp.DecodeBytes(common.FromHex(test.Input), tx); err != nil {
			...
			statedb := tests.MakePreState(sofdb.NewMemDatabase(), test.Genesis.Alloc)
			...
			svm := vm.NewSVM(context, statedb, test.Genesis.Config, vm.Config{Debug: true, Tracer: tracer})
