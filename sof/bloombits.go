...
const (
	// bloomServiceThreads is the number of goroutines used globally by an Sophon
...
func (sof *Sophon) startBloomHandlers(sectionSize uint64) {
	for i := 0; i < bloomServiceThreads; i++ {
		go func() {
			for {
				select {
				case <-sof.shutdownChan:
					return

				case request := <-sof.bloomRequests:
					task := <-request
					task.Bitsets = make([][]byte, len(task.Sections))
					for i, section := range task.Sections {
						head := rawdb.ReadCanonicalHash(sof.chainDb, (section+1)*sectionSize-1)
						if compVector, err := rawdb.ReadBloomBits(sof.chainDb, task.Bit, section, head); err == nil {
...
// for the Sophon header bloom filters, permitting blazing fast filtering.
type BloomIndexer struct {
	size    uint64               // section size to generate bloombits for
	db      sofdb.Database       // database instance to write index data and metadata into
...
func NewBloomIndexer(db sofdb.Database, size, confirms uint64) *core.ChainIndexer {
	backend := &BloomIndexer{
		db:   db,
		size: size,
	}
	table := sofdb.NewTable(db, string(rawdb.BloomBitsIndexPrefix))
