...
var (
	...
	maxQueuedHeaders         = 32 * 1024                    // [sof/62] Maximum number of headers to queue for import (DOS protection)
...
type Downloader struct {
	...
	stateDB    sofdb.Database  // Database to state sync into (and deduplicate via)
	...
	// Channels
	headerCh      chan dataPack        // [sof/62] Channel receiving inbound block headers
	bodyCh        chan dataPack        // [sof/62] Channel receiving inbound block bodies
	receiptCh     chan dataPack        // [sof/63] Channel receiving inbound receipts
	bodyWakeCh    chan bool            // [sof/62] Channel to signal the block body fetcher of new tasks
	receiptWakeCh chan bool            // [sof/63] Channel to signal the receipt fetcher of new tasks
	headerProcCh  chan []*types.Header // [sof/62] Channel to feed the header processor new tasks

	// for stateFetcher
	stateSyncStart chan *stateSync
	trackStateReq  chan *stateReq
	stateCh        chan dataPack // [sof/63] Channel receiving inbound node state data
...
func New(checkpoint uint64, stateDb sofdb.Database, stateBloom *trie.SyncBloom, mux *event.TypeMux, chain BlockChain, lightchain LightChain, dropPeer peerDropFn) *Downloader {
	if lightchain == nil {
		lightchain = chain
	}
	dl := &Downloader{
...
// these are zero.
func (d *Downloader) Progress() sophon.SyncProgress {
...
func (d *Downloader) syncWithPeer(p *peerConnection, hash common.Hash, td *big.Int) (err error) {
	...
	log.Debug("Synchronising with the network", "peer", p.id, "sof", p.version, "head", hash, "td", td, "mode", d.mode)
...
func (d *Downloader) importBlockResults(results []*fetchResult) error {
	...
	if index, err := d.blockchain.InsertChain(blocks); err != nil {
		if index < len(results) {
			...
			// The importer will put togsophy a new list of blocks to import, which is a superset
