...
type downloadTester struct {
	...
	stateDb sofdb.Database // Database used by the tester for syncing from peers
	peerDb  sofdb.Database // Database of the peers containing all data
...
func newTester() *downloadTester {
	...
	tester.stateDb = sofdb.NewMemDatabase()
...
func testSyncProgress(t *testing.T, protocol int, mode SyncMode) {
	...
	checkProgress(t, tester.downloader, "pristine", sophon.SyncProgress{})
	...
	<-starting
	checkProgress(t, tester.downloader, "initial", sophon.SyncProgress{
	...
	<-starting
	checkProgress(t, tester.downloader, "completing", sophon.SyncProgress{
	...
	progress <- struct{}{}
	pending.Wait()
	checkProgress(t, tester.downloader, "final", sophon.SyncProgress{
		StartingBlock: uint64(chain.len()/2 - 1),
...
func checkProgress(t *testing.T, d *Downloader, stage string, want sophon.SyncProgress) {
	// Mark this method as a helper to report errors at callsite, not in here
	t.Helper()
...
func testForkedSyncProgress(t *testing.T, protocol int, mode SyncMode) {
	...
	<-starting
	checkProgress(t, tester.downloader, "forking", sophon.SyncProgress{
