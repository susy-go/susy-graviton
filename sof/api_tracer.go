...
import (
	...
	"github.com/susy-go/susy-graviton/sof/tracers"
	...
	"github.com/susy-go/susy-graviton/trie"
)
...
// TraceChain returns the structured logs created during the execution of SVM
...
func (api *PrivateDebugAPI) TraceChain(ctx context.Context, start, end rpc.BlockNumber, config *TraceConfig) (*rpc.Subscription, error) {
	...
	case rpc.PendingBlockNumber:
		from = api.sof.miner.PendingBlock()
	case rpc.LatestBlockNumber:
		from = api.sof.blockchain.CurrentBlock()
	default:
		from = api.sof.blockchain.GetBlockByNumber(uint64(start))
	}
	switch end {
	case rpc.PendingBlockNumber:
		to = api.sof.miner.PendingBlock()
	case rpc.LatestBlockNumber:
		to = api.sof.blockchain.CurrentBlock()
	default:
		to = api.sof.blockchain.GetBlockByNumber(uint64(end))
	}
...
func (api *PrivateDebugAPI) traceChain(ctx context.Context, start, end *types.Block, config *TraceConfig) (*rpc.Subscription, error) {
	...
	database := state.NewDatabaseWithCache(api.sof.ChainDb(), 16) // Chain tracing will probably start at genesis

	if number := start.NumberU64(); number > 0 {
		start = api.sof.blockchain.GetBlock(start.ParentHash(), start.NumberU64()-1)
		if start == nil {
	...
	for th := 0; th < threads; th++ {
		...
		go func() {
			...
			for task := range tasks {
				signer := types.MakeSigner(api.sof.blockchain.Config(), task.block.Number())

				// Trace all the transactions contained within
				for i, tx := range task.block.Transactions() {
					msg, _ := tx.AsMessage(signer)
					vmctx := core.NewSVMContext(msg, task.block.Header(), api.sof.blockchain, nil)
					...
					task.statedb.Finalise(api.sof.blockchain.Config().IsSIP158(task.block.Number()))
	...
	go func() {
		...
		for number = start.NumberU64() + 1; number <= end.NumberU64(); number++ {
			...
			block := api.sof.blockchain.GetBlockByNumber(number)
			if block == nil {
			...
			_, _, _, err := api.sof.blockchain.Processor().Process(block, statedb, vm.Config{})
			if err != nil {
				failed = err
				break
			}
			// Finalize the state so any modifications are written to the trie
			root, err := statedb.Commit(api.sof.blockchain.Config().IsSIP158(block.Number()))
			if err != nil {
				failed = err
				break
			}
...
// SVM and returns them as a JSON object.
func (api *PrivateDebugAPI) TraceBlockByNumber(ctx context.Context, number rpc.BlockNumber, config *TraceConfig) ([]*txTraceResult, error) {
	...
	switch number {
	case rpc.PendingBlockNumber:
		block = api.sof.miner.PendingBlock()
	case rpc.LatestBlockNumber:
		block = api.sof.blockchain.CurrentBlock()
	default:
		block = api.sof.blockchain.GetBlockByNumber(uint64(number))
	}
...
func (api *PrivateDebugAPI) TraceBlockByHash(ctx context.Context, hash common.Hash, config *TraceConfig) ([]*txTraceResult, error) {
	block := api.sof.blockchain.GetBlockByHash(hash)
	if block == nil {
...
// TraceBlock returns the structured logs created during the execution of SVM
// and returns them as a JSON object.
func (api *PrivateDebugAPI) TraceBlock(ctx context.Context, blob []byte, config *TraceConfig) ([]*txTraceResult, error) {
	block := new(types.Block)
	if err := srlp.Decode(bytes.NewReader(blob), block); err != nil {
...
// SVM against a block pulled from the pool of bad ones and returns them as a JSON
// object.
func (api *PrivateDebugAPI) TraceBadBlock(ctx context.Context, hash common.Hash, config *TraceConfig) ([]*txTraceResult, error) {
	blocks := api.sof.blockchain.BadBlocks()
...
// execution of SVM to the local file system and returns a list of files
// to the caller.
...
// execution of SVM against a block pulled from the pool of bad ones to the
...
func (api *PrivateDebugAPI) traceBlock(ctx context.Context, block *types.Block, config *TraceConfig) ([]*txTraceResult, error) {
	...
	if err := api.sof.engine.VerifyHeader(api.sof.blockchain, block.Header(), true); err != nil {
		return nil, err
	}
	parent := api.sof.blockchain.GetBlock(block.ParentHash(), block.NumberU64()-1)
	if parent == nil {
	...
	var (
		signer = types.MakeSigner(api.sof.blockchain.Config(), block.Number())
	...
	for th := 0; th < threads; th++ {
		...
		go func() {
			...
			for task := range jobs {
				...
				vmctx := core.NewSVMContext(msg, block.Header(), api.sof.blockchain, nil)
	...
	for i, tx := range txs {
		...
		vmenv := vm.NewSVM(vmctx, statedb, api.sof.blockchain.Config(), vm.Config{})
		...
		// Finalize the state so any modifications are written to the trie
		// Only delete empty objects if SIP158/161 (a.k.a Spurious Dragon) is in effect
		statedb.Finalise(vmenv.ChainConfig().IsSIP158(block.Number()))
	}
	close(jobs)
	pend.Wait()
...
func (api *PrivateDebugAPI) standardTraceBlockToFile(ctx context.Context, block *types.Block, config *StdTraceConfig) ([]string, error) {
	...
	for i, tx := range block.Transactions() {
		...
		vmenv := vm.NewSVM(vmctx, statedb, api.sof.blockchain.Config(), vmConf)
...
func (api *PrivateDebugAPI) computeStateDB(block *types.Block, reexec uint64) (*state.StateDB, error) {
	...
	statedb, err := api.sof.blockchain.StateAt(block.Root())
	if err == nil {
		return statedb, nil
	}
	// Otherwise try to reexec blocks until we find a state or reach our limit
	origin := block.NumberU64()
	database := state.NewDatabaseWithCache(api.sof.ChainDb(), 16)

	for i := uint64(0); i < reexec; i++ {
		block = api.sof.blockchain.GetBlock(block.ParentHash(), block.NumberU64()-1)
		if block == nil {
			break
		}
	...
	for block.NumberU64() < origin {
		...
		if block = api.sof.blockchain.GetBlockByNumber(block.NumberU64() + 1); block == nil {
...
// TraceTransaction returns the structured logs created during the execution of SVM
// and returns them as a JSON object.
func (api *PrivateDebugAPI) TraceTransaction(ctx context.Context, hash common.Hash, config *TraceConfig) (interface{}, error) {
	// Retrieve the transaction and assemble its SVM context
	tx, blockHash, _, index := rawdb.ReadTransaction(api.sof.ChainDb(), hash)
	if tx == nil {
...
func (api *PrivateDebugAPI) traceTx(ctx context.Context, message core.Message, vmctx vm.Context, statedb *state.StateDB, config *TraceConfig) (interface{}, error) {
	...
	vmenv := vm.NewSVM(vmctx, statedb, api.sof.blockchain.Config(), vm.Config{Debug: true, Tracer: tracer})
	...
	case *vm.StructLogger:
		return &sofapi.ExecutionResult{
			Gas:         gas,
			Failed:      failed,
			ReturnValue: fmt.Sprintf("%x", ret),
			StructLogs:  sofapi.FormatLogs(tracer.StructLogs()),
		}, nil
...
func (api *PrivateDebugAPI) computeTxEnv(blockHash common.Hash, txIndex int, reexec uint64) (core.Message, vm.Context, *state.StateDB, error) {
	// Create the parent state database
	block := api.sof.blockchain.GetBlockByHash(blockHash)
	if block == nil {
	...
	for idx, tx := range block.Transactions() {
		...
		msg, _ := tx.AsMessage(signer)
		context := core.NewSVMContext(msg, block.Header(), api.sof.blockchain, nil)
		if idx == txIndex {
			return msg, context, statedb, nil
		}
		// Not yet the searched for transaction, execute on top of the current state
		vmenv := vm.NewSVM(context, statedb, api.sof.blockchain.Config(), vm.Config{})
