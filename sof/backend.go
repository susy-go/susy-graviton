...
func New(ctx *node.ServiceContext, config *Config) (*Sophon, error) {
	...
	sof := &Sophon{
		...
		engine:         CreateConsensusEngine(ctx, chainConfig, &config.Sofash, config.MinerNotify, config.MinerNoverify, chainDb),
	...
	log.Info("Initialising Sophon protocol", "versions", ProtocolVersions, "network", config.NetworkId)
	...
	sof.blockchain, err = core.NewBlockChain(chainDb, cacheConfig, sof.chainConfig, sof.engine, vmConfig, sof.shouldPreserve)
	if err != nil {
		return nil, err
	}
	...
	sof.txPool = core.NewTxPool(config.TxPool, sof.chainConfig, sof.blockchain)

	if sof.protocolManager, err = NewProtocolManager(sof.chainConfig, config.SyncMode, config.NetworkId, sof.eventMux, sof.txPool, sof.engine, sof.blockchain, chainDb, config.Whitelist); err != nil {
		return nil, err
	}

	sof.miner = miner.New(sof, sof.chainConfig, sof.EventMux(), sof.engine, config.MinerRecommit, config.MinerGasFloor, config.MinerGasCeil, sof.isLocalBlock)
	sof.miner.SetExtra(makeExtraData(config.MinerExtraData))

	sof.APIBackend = &SofAPIBackend{sof, nil}
...
func CreateDB(ctx *node.ServiceContext, config *Config, name string) (sofdb.Database, error) {
	db, err := ctx.OpenDatabase(name, config.DatabaseCache, config.DatabaseHandles)
	if err != nil {
		return nil, err
	}
	if db, ok := db.(*sofdb.LDBDatabase); ok {
		db.Meter("sof/db/chaindata/")
	}
	return db, nil
}
