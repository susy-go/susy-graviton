...
func newTestProtocolManager(mode downloader.SyncMode, blocks int, generator func(int, *core.BlockGen), newtx chan<- []*types.Transaction) (*ProtocolManager, *sofdb.MemDatabase, error) {
	var (
	...
	chain, _ := core.GenerateChain(gspec.Config, genesis, sofash.NewFaker(), db, blocks, generator)
	if _, err := blockchain.InsertChain(chain); err != nil {
		panic(err)
	}

	pm, err := NewProtocolManager(gspec.Config, mode, DefaultConfig.NetworkId, svmux, &testTxPool{added: newtx}, engine, blockchain, db, nil)
	if err != nil {
		return nil, nil, err
	}
	pm.Start(1000)
...
// fails the test.
func newTestProtocolManagerMust(t *testing.T, mode downloader.SyncMode, blocks int, generator func(int, *core.BlockGen), newtx chan<- []*types.Transaction) (*ProtocolManager, *sofdb.MemDatabase) {
