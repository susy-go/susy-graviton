...
var (
	...
	MainnetChainConfig = &ChainConfig{
		...
		SIP150Hash:          common.HexToHash("0x2086799aeebeae135c246c65021c82b4e15a2c451340993aacfd2751886514f0"),
		SIP155Block:         big.NewInt(2675000),
		SIP158Block:         big.NewInt(2675000),
		ByzantiumBlock:      big.NewInt(4370000),
		ConstantinopleBlock: big.NewInt(7280000),
		PetersburgBlock:     big.NewInt(7280000),
		Sofash:              new(SofashConfig),
	}
	...
	TestnetChainConfig = &ChainConfig{
		...
		SIP150Hash:          common.HexToHash("0x41941023680923e0fe4d74a34bdac8141f2540e3ae90623718e47d66d1ca4a2d"),
	...
	RinkebyChainConfig = &ChainConfig{
		...
		SIP150Hash:          common.HexToHash("0x9b095b36c15eaf13044373aef8ee0bd3a382a5abb92e402afa44b8249c3a90e9"),
		SIP155Block:         big.NewInt(3),
		SIP158Block:         big.NewInt(3),
	...
	// AllSofashProtocolChanges contains every protocol change (SIPs) introduced
	// and accepted by the Sophon core developers into the Sofash consensus.
	//
	// This configuration is intentionally not using keyed fields to force anyone
	// adding flags to the config to also have to set these fields.
	AllSofashProtocolChanges = &ChainConfig{big.NewInt(1337), big.NewInt(0), nil, false, big.NewInt(0), common.Hash{}, big.NewInt(0), big.NewInt(0), big.NewInt(0), big.NewInt(0), big.NewInt(0), nil, new(SofashConfig), nil}

	// AllCliqueProtocolChanges contains every protocol change (SIPs) introduced
	// and accepted by the Sophon core developers into the Clique consensus.
	//
	// This configuration is intentionally not using keyed fields to force anyone
	// adding flags to the config to also have to set these fields.
	...
	TestChainConfig = &ChainConfig{big.NewInt(1), big.NewInt(0), nil, false, big.NewInt(0), common.Hash{}, big.NewInt(0), big.NewInt(0), big.NewInt(0), big.NewInt(0), big.NewInt(0), nil, new(SofashConfig), nil}
...
type ChainConfig struct {
	...
	// SIP150 implements the Gas price changes (https://github.com/susy-go/SIPs/issues/150)
	SIP150Block *big.Int    `json:"sip150Block,omitempty"` // SIP150 HF block (nil = no fork)
	SIP150Hash  common.Hash `json:"sip150Hash,omitempty"`  // SIP150 HF hash (needed for header only clients as only gas pricing changed)
	...
	Sofash *SofashConfig `json:"sofash,omitempty"`
	Clique *CliqueConfig `json:"clique,omitempty"`
}

// SofashConfig is the consensus engine configs for proof-of-work based sealing.
type SofashConfig struct{}

// String implements the stringer interface, returning the consensus engine details.
func (c *SofashConfig) String() string {
	return "sofash"
}
...
func (c *ChainConfig) String() string {
	...
	return fmt.Sprintf("{ChainID: %v Homestead: %v DAO: %v DAOSupport: %v SIP150: %v SIP155: %v SIP158: %v Byzantium: %v Constantinople: %v  ConstantinopleFix: %v Engine: %v}",
		c.ChainID,
		c.HomesteadBlock,
		c.DAOForkBlock,
		c.DAOForkSupport,
...
func (c *ChainConfig) IsSIP150(num *big.Int) bool {
	return isForked(c.SIP150Block, num)
}
...
func (c *ChainConfig) IsSIP155(num *big.Int) bool {
	return isForked(c.SIP155Block, num)
}
...
func (c *ChainConfig) IsSIP158(num *big.Int) bool {
	return isForked(c.SIP158Block, num)
}
...
func (c *ChainConfig) GasTable(num *big.Int) GasTable {
	...
	case c.IsSIP158(num):
		return GasTableSIP158
	case c.IsSIP150(num):
		return GasTableSIP150
	default:
		return GasTableHomestead
	}
}
...
func (c *ChainConfig) checkCompatible(newcfg *ChainConfig, head *big.Int) *ConfigCompatError {
	...
	if isForkIncompatible(c.SIP150Block, newcfg.SIP150Block, head) {
		return newCompatError("SIP150 fork block", c.SIP150Block, newcfg.SIP150Block)
	}
	if isForkIncompatible(c.SIP155Block, newcfg.SIP155Block, head) {
		return newCompatError("SIP155 fork block", c.SIP155Block, newcfg.SIP155Block)
	}
	if isForkIncompatible(c.SIP158Block, newcfg.SIP158Block, head) {
		return newCompatError("SIP158 fork block", c.SIP158Block, newcfg.SIP158Block)
	}
	if c.IsSIP158(head) && !configNumEqual(c.ChainID, newcfg.ChainID) {
		return newCompatError("SIP158 chain ID", c.SIP158Block, newcfg.SIP158Block)
	}
