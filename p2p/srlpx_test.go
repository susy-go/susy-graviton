...
import (
	...
	"github.com/susy-go/susy-graviton/p2p/simulations/pipes"
...
func testEncHandshake(token []byte) error {
	...
	var (
		...
		prv1, _  = crypto.GenerateKey()
		fd0, fd1 = net.Pipe()
		c0, c1   = newRLPX(fd0).(*srlpx), newRLPX(fd1).(*srlpx)
...
func TestProtocolHandshake(t *testing.T) {
	...
	go func() {
		...
		defer fd0.Close()
		srlpx := newRLPX(fd0)
		rpubkey, err := srlpx.doEncHandshake(prv0, &prv1.PublicKey)
		if err != nil {
		...
		phs, err := srlpx.doProtoHandshake(hs0)
		if err != nil {
		...
		srlpx.close(DiscQuitting)
	}()
	go func() {
		defer wg.Done()
		defer fd1.Close()
		srlpx := newRLPX(fd1)
		rpubkey, err := srlpx.doEncHandshake(prv1, nil)
		if err != nil {
		...
		phs, err := srlpx.doProtoHandshake(hs1)
		if err != nil {
		...
		if err := ExpectMsg(srlpx, discMsg, []DiscReason{DiscQuitting}); err != nil {
...
func TestProtocolHandshakeErrors(t *testing.T) {
	tests := []struct {
		...
		},
		{
			code: handshakeMsg,
			msg:  []byte{1, 2, 3},
			err:  newPeerError(errInvalidMsg, "(code 0) (size 4) srlp: expected input list for p2p.protoHandshake"),
		},
		{
			code: handshakeMsg,
...
func TestRLPXFrameRW(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		...
		payload, _ := ioutil.ReadAll(msg.Payload)
		wantPayload, _ := srlp.EncodeToBytes(wmsg)
		if !bytes.Equal(payload, wantPayload) {
...
var sip8HandshakeAuthTests = []handshakeAuthTest{
	...
	// (Auth₃) RLPx v4 SIP-8 encoding with version 56, additional list elements
	{
		...
		`,
		wantVersion: 56,
		wantRest:    []srlp.RawValue{{0x01}, {0x02}, {0xC2, 0x04, 0x05}},
	},
}
...
var sip8HandshakeRespTests = []handshakeAckTest{
	...
	// (Ack₃) SIP-8 encoding with version 57, additional list elements
	{
		...
		`,
		wantVersion: 57,
		wantRest:    []srlp.RawValue{{0x06}, {0xC2, 0x07, 0x08}, {0x81, 0xFA}},
	},
}
...
func TestHandshakeForwardCompatibility(t *testing.T) {
	...
	var (
		...
		authCiphertext     = unhex(sip8HandshakeAuthTests[1].input)
		authRespCiphertext = unhex(sip8HandshakeRespTests[1].input)
		authMsg            = makeAuth(sip8HandshakeAuthTests[1])
