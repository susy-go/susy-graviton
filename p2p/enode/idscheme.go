...
func SignV4(r *enr.Record, privkey *ecdsa.PrivateKey) error {
	...
	srlp.Encode(h, cpy.AppendElements(nil))
...
func (V4ID) Verify(r *enr.Record, sig []byte) error {
	...
	h := sha3.NewLegacyKeccak256()
	srlp.Encode(h, r.AppendElements(nil))
...
func (v Secp256k1) EncodeRLP(w io.Writer) error {
	return srlp.Encode(w, crypto.CompressPubkey((*ecdsa.PublicKey)(&v)))
}

// DecodeRLP implements srlp.Decoder.
func (v *Secp256k1) DecodeRLP(s *srlp.Stream) error {
	buf, err := s.Bytes()
	if err != nil {
		return err
	}
