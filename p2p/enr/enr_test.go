...
func TestDirty(t *testing.T) {
	...
	if _, err := srlp.EncodeToBytes(r); err != errEncodeUnsigned {
...
func TestSignEncodeAndDecode(t *testing.T) {
	...
	blob, err := srlp.EncodeToBytes(r)
	require.NoError(t, err)

	var r2 Record
	require.NoError(t, srlp.DecodeBytes(blob, &r2))
	assert.Equal(t, r, r2)

	blob2, err := srlp.EncodeToBytes(r2)
	require.NoError(t, err)
