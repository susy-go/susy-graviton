...
// also implement srlp.Decoder if additional checks are needed on the value.
...
func (g generic) EncodeRLP(w io.Writer) error {
	return srlp.Encode(w, g.value)
}

func (g *generic) DecodeRLP(s *srlp.Stream) error {
	return s.Decode(g.value)
}

// WithEntry wraps any value with a key name. It can be used to set and load arbitrary values
// in a record. The value v must be supported by srlp. To use WithEntry with Load, the value
// must be a pointer.
...
func (v IP) EncodeRLP(w io.Writer) error {
	if ip4 := net.IP(v).To4(); ip4 != nil {
		return srlp.Encode(w, ip4)
	}
	return srlp.Encode(w, net.IP(v))
}

// DecodeRLP implements srlp.Decoder.
func (v *IP) DecodeRLP(s *srlp.Stream) error {
