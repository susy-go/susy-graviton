...
// Package enr implements Sophon Node Records as defined in SIP-778. A node record holds
...
func (r *Record) Load(e Entry) error {
	...
	if i < len(r.pairs) && r.pairs[i].k == e.ENRKey() {
		if err := srlp.DecodeBytes(r.pairs[i].v, e); err != nil {
...
// EncodeRLP implements srlp.Encoder. Encoding fails if
...
// DecodeRLP implements srlp.Decoder. Decoding verifies the signature.
func (r *Record) DecodeRLP(s *srlp.Stream) error {
...
func decodeRecord(s *srlp.Stream) (dec Record, raw []byte, err error) {
	raw, err = s.Raw()
	if err != nil {
		return dec, raw, err
	}
	...
	s = srlp.NewStream(bytes.NewReader(raw), 0)
...
func (r *Record) encode(sig []byte) (raw []byte, err error) {
	...
	if raw, err = srlp.EncodeToBytes(list); err != nil {
		return nil, err
	}
	if len(raw) > SizeLimit {
