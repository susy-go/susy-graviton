...
func (t *UDPv4) makePing(toaddr *net.UDPAddr) *pingV4 {
	seq, _ := srlp.EncodeToBytes(t.localNode.Node().Seq())
	return &pingV4{
		Version:    4,
		From:       t.ourEndpoint(),
		To:         makeEndpoint(toaddr, 0),
		Expiration: uint64(time.Now().Add(expiration).Unix()),
		Rest:       []srlp.RawValue{seq},
	}
}
...
func init() {
	...
	for n := 0; ; n++ {
		...
		size, _, err := srlp.EncodeToReader(p)
		if err != nil {
...
func (t *UDPv4) encode(priv *ecdsa.PrivateKey, req packetV4) (packet, hash []byte, err error) {
	...
	if err := srlp.Encode(b, req); err != nil {
...
func decodeV4(buf []byte) (packetV4, encPubkey, []byte, error) {
	...
	s := srlp.NewStream(bytes.NewReader(sigdata[1:]), 0)
	err = s.Decode(req)
...
func seqFromTail(tail []srlp.RawValue) uint64 {
	if len(tail) == 0 {
		return 0
	}
	var seq uint64
	srlp.DecodeBytes(tail[0], &seq)
	return seq
}
