...
import (
	...
	"github.com/susy-go/susy-graviton/common/bitutil"
	"github.com/susy-go/susy-graviton/crypto"
	"github.com/susy-go/susy-graviton/crypto/ecies"
	"github.com/susy-go/susy-graviton/crypto/secp256k1"
...
const (
	...
	encAuthMsgLen  = authMsgLen + eciesOverhead  // size of encrypted pre-SIP-8 initiator handshake
	encAuthRespLen = authRespLen + eciesOverhead // size of encrypted pre-SIP-8 handshake reply
...
func (t *srlpx) ReadMsg() (Msg, error) {
	t.rmu.Lock()
	defer t.rmu.Unlock()
...
func (t *srlpx) WriteMsg(msg Msg) error {
	t.wmu.Lock()
	defer t.wmu.Unlock()
...
func (t *srlpx) close(err error) {
	t.wmu.Lock()
	defer t.wmu.Unlock()
	// Tell the remote end why we're disconnecting if possible.
	if t.rw != nil {
		if r, ok := err.(DiscReason); ok && r != DiscNetworkError {
			// srlpx tries to send DiscReason to disconnected peer
...
func (t *srlpx) doProtoHandshake(our *protoHandshake) (their *protoHandshake, err error) {
...
func readProtocolHandshake(rw MsgReader, our *protoHandshake) (*protoHandshake, error) {
	...
	if msg.Code == discMsg {
		...
		srlp.Decode(msg.Payload, &reason)
		return nil, reason[0]
	}
...
func (t *srlpx) doEncHandshake(prv *ecdsa.PrivateKey, dial *ecdsa.PublicKey) (*ecdsa.PublicKey, error) {
	var (
		sec secrets
		err error
	)
...
func initiatorEncHandshake(conn io.ReadWriter, prv *ecdsa.PrivateKey, remote *ecdsa.PublicKey) (s secrets, err error) {
	...
	authPacket, err := sealSIP8(authMsg, h)
	if err != nil {
		return s, err
	}
...
func receiverEncHandshake(conn io.ReadWriter, prv *ecdsa.PrivateKey) (s secrets, err error) {
	...
	if authMsg.gotPlain {
		...
		authRespPacket, err = sealSIP8(authRespMsg, h)
	}
	if err != nil {
		return s, err
	}
...
func sealSIP8(msg interface{}, h *encHandshake) ([]byte, error) {
	buf := new(bytes.Buffer)
	if err := srlp.Encode(buf, msg); err != nil {
		return nil, err
	}
...
func readHandshakeMsg(msg plainDecoder, plainSize int, prv *ecdsa.PrivateKey, r io.Reader) ([]byte, error) {
	...
	// Attempt decoding pre-SIP-8 "plain" format.
	key := ecies.ImportECDSA(prv)
	if dec, err := key.Decrypt(buf, nil, nil); err == nil {
		msg.decodePlain(dec)
		return buf, nil
	}
	// Could be SIP-8 format, try that.
	prefix := buf[:2]
	...
	// Can't use srlp.DecodeBytes here because it rejects
	// trailing data (forward-compatibility).
	s := srlp.NewStream(bytes.NewReader(dec), 0)
...
func newRLPXFrameRW(conn io.ReadWriter, s secrets) *srlpxFrameRW {
...
func (rw *srlpxFrameRW) WriteMsg(msg Msg) error {
	ptype, _ := srlp.EncodeToBytes(msg.Code)
...
func (rw *srlpxFrameRW) ReadMsg() (msg Msg, err error) {
	// read the header
	headbuf := make([]byte, 32)
	...
	if err := srlp.Decode(content, &msg.Code); err != nil {
		return msg, err
	}
