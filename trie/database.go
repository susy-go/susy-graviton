...
type Database struct {
	diskdb sofdb.KeyValueStore // Persistent storage for matured trie nodes
...
type cachedNode struct {
	node node   // Cached collapsed trie node, or raw srlp data
...
// srlp returns the raw srlp encoded blob of the cached node, either directly from
// the cache, or by regenerating it from the collapsed node.
func (n *cachedNode) srlp() []byte {
	if node, ok := n.node.(rawNode); ok {
		return node
	}
	blob, err := srlp.EncodeToBytes(n.node)
	if err != nil {
		panic(err)
	}
	return blob
...
// or by regenerating it from the srlp encoded blob.
...
func NewDatabase(diskdb sofdb.KeyValueStore) *Database {
...
func NewDatabaseWithCache(diskdb sofdb.KeyValueStore, cache int) *Database {
...
func (db *Database) DiskDB() sofdb.KeyValueReader {
	return db.diskdb
}
...
func (db *Database) Cap(limit common.StorageSize) error {
	...
	for size > limit && oldest != (common.Hash{}) {
		...
		if err := batch.Put(oldest[:], node.srlp()); err != nil {
			return err
		}
...
func (db *Database) commit(hash common.Hash, batch sofdb.Batch, uncacher *cleaner) error {
	...
	if err := batch.Put(hash[:], node.srlp()); err != nil {
		return err
	}
...
// memory to disk.
func (c *cleaner) Put(key []byte, srlp []byte) error {
	...
	if c.db.cleans != nil {
		c.db.cleans.Set(string(hash[:]), srlp)
	}
	return nil
}
