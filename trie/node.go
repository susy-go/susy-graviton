...
func decodeNode(hash, buf []byte, cachegen uint16) (node, error) {
	...
	elems, _, err := srlp.SplitList(buf)
	if err != nil {
		return nil, fmt.Errorf("decode error: %v", err)
	}
	switch c, _ := srlp.CountValues(elems); c {
	case 2:
...
func decodeShort(hash, elems []byte, cachegen uint16) (node, error) {
	kbuf, rest, err := srlp.SplitString(elems)
	if err != nil {
		return nil, err
	}
	...
	if hasTerm(key) {
		// value node
		val, _, err := srlp.SplitString(rest)
		if err != nil {
...
func decodeFull(hash, elems []byte, cachegen uint16) (*fullNode, error) {
	...
	val, _, err := srlp.SplitString(elems)
	if err != nil {
		return n, err
	}
	if len(val) > 0 {
...
func decodeRef(buf []byte, cachegen uint16) (node, []byte, error) {
	kind, val, rest, err := srlp.Split(buf)
	if err != nil {
		return nil, buf, err
	}
	switch {
	case kind == srlp.List:
		...
		return n, rest, err
	case kind == srlp.String && len(val) == 0:
		// empty node
		return nil, rest, nil
	case kind == srlp.String && len(val) == 32:
