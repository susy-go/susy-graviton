...
// Used for testing
func newEmpty() *Trie {
	trie, _ := New(common.Hash{}, NewDatabase(sofdb.NewMemDatabase()))
	return trie
}
...
func TestMissingRoot(t *testing.T) {
	trie, err := New(common.HexToHash("0beec7b5ea3f0fdbc95d0dd47f3c5bc275da8a33"), NewDatabase(sofdb.NewMemDatabase()))
	if trie != nil {
...
func runRandTest(rt randTest) bool {
	triedb := NewDatabase(sofdb.NewMemDatabase())
	...
	for i, step := range rt {
		...
		case opItsrcheckhash:
			...
			if tr.Hash() != checktr.Hash() {
				rt[i].err = fmt.Errorf("hash mismatch in opItsrcheckhash")
			}
...
func benchGet(b *testing.B, commit bool) {
	...
	if commit {
		ldb := trie.db.diskdb.(*sofdb.LDBDatabase)
		ldb.Close()
...
func BenchmarkHash(b *testing.B) {
	...
	for i := 0; i < len(accounts); i++ {
		...
		accounts[i], _ = srlp.EncodeToBytes([]interface{}{nonce, balance, root, code})
	}
...
func tempDB() (string, *Database) {
	...
	diskdb, err := sofdb.NewLDBDatabase(dir, 256, 0)
	if err != nil {
