...
func NewLesServer(sof *sof.Sophon, config *sof.Config) (*LesServer, error) {
	quitSync := make(chan struct{})
	pm, err := NewProtocolManager(sof.BlockChain().Config(), light.DefaultServerIndexerConfig, false, config.NetworkId, sof.EventMux(), sof.Engine(), newPeerSet(), sof.BlockChain(), sof.TxPool(), sof.ChainDb(), nil, nil, nil, quitSync, new(sync.WaitGroup))
	if err != nil {
		return nil, err
	}
	...
	for i, pv := range AdvertiseProtocolVersions {
		lesTopics[i] = lesTopic(sof.BlockChain().Genesis().Hash(), pv)
	}
	...
	srv := &LesServer{
		lesCommons: lesCommons{
			...
			chtIndexer:       light.NewChtIndexer(sof.ChainDb(), nil, params.CHTFrequencyServer, params.HelperTrieProcessConfirmations),
			bloomTrieIndexer: light.NewBloomTrieIndexer(sof.ChainDb(), nil, params.BloomBitsBlocks, params.BloomTrieFrequency),
			protocolManager:  pm,
		},
		quitSync:  quitSync,
		lesTopics: lesTopics,
	...
	srv.chtIndexer.Start(sof.BlockChain())
	pm.server = srv
	...
	srv.fcCostStats = newCostStats(sof.ChainDb())
	return srv, nil
}
...
func newCostStats(db sofdb.Database) *requestCostStats {
	...
	if db != nil {
		...
		if err == nil {
			err = srlp.DecodeBytes(data, &statsRlp)
		}
		if err == nil {
...
func (s *requestCostStats) store() {
	...
	if data, err := srlp.EncodeToBytes(statsRlp); err == nil {
