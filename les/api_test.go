...
import (
	...
	"github.com/susy-go/susy-graviton/p2p/simulations"
	"github.com/susy-go/susy-graviton/p2p/simulations/adapters"
...
func getHead(ctx context.Context, t *testing.T, client *rpc.Client) (uint64, common.Hash) {
	...
	if err := client.CallContext(ctx, &res, "sof_getBlockByNumber", "latest", false); err != nil {
...
func testRequest(ctx context.Context, t *testing.T, client *rpc.Client) bool {
	...
	//	if err := client.CallContext(ctx, &res, "sof_getProof", addr, nil, "latest"); err != nil {
	err := client.CallContext(c, &res, "sof_getBalance", addr, "latest")
	if err != nil {
...
func testSim(t *testing.T, serverCount, clientCount int, serverDir, clientDir []string, test func(ctx context.Context, net *simulations.Network, servers []*simulations.Node, clients []*simulations.Node) bool) bool {
	...
	for i := range servers {
		servsrconf := adapters.RandomNodeConfig()
		servsrconf.Services = []string{"lesserver"}
		if len(serverDir) == serverCount {
			servsrconf.DataDir = serverDir[i]
		}
		server, err := net.NewNodeWithConfig(servsrconf)
		if err != nil {
...
func newLesServerService(ctx *adapters.ServiceContext) (node.Service, error) {
	...
	sophon, err := sof.New(ctx.NodeContext, &config)
	if err != nil {
		return nil, err
	}

	server, err := NewLesServer(sophon, &config)
	if err != nil {
		return nil, err
	}
	sophon.AddLesServer(server)
	return sophon, nil
}
