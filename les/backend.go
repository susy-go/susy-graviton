...
func New(ctx *node.ServiceContext, config *sof.Config) (*LightSophon, error) {
	chainDb, err := ctx.OpenDatabase("lightchaindata", config.DatabaseCache, config.DatabaseHandles, "sof/db/chaindata/")
	if err != nil {
		return nil, err
	}
	...
	if lsof.config.ULC != nil {
		trustedNodes = lsof.config.ULC.TrustedServers
	}
	lsof.serverPool = newServerPool(chainDb, quitSync, &lsof.wg, trustedNodes)
	lsof.retriever = newRetrieveManager(peers, lsof.reqDist, lsof.serverPool)
	lsof.relay = NewLesTxRelay(peers, lsof.retriever)

	lsof.odr = NewLesOdr(chainDb, light.DefaultClientIndexerConfig, lsof.retriever)
	lsof.chtIndexer = light.NewChtIndexer(chainDb, lsof.odr, params.CHTFrequency, params.HelperTrieConfirmations)
	...
	if lsof.protocolManager, err = NewProtocolManager(
		lsof.chainConfig,
	...
	if lsof.protocolManager.isULCEnabled() {
		log.Warn("Ultra light client is enabled", "trustedNodes", len(lsof.protocolManager.ulc.trustedKeys), "minTrustedFraction", lsof.protocolManager.ulc.minTrustedFraction)
		lsof.blockchain.DisableCheckFreq()
	}
	lsof.ApiBackend = &LesApiBackend{ctx.ExtRPCEnabled(), lsof, nil}
