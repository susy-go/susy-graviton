...
func (p *peer) ReplyBlockHeaders(reqID uint64, headers []*types.Header) *reply {
	data, _ := srlp.EncodeToBytes(headers)
	return &reply{p.rw, BlockHeadersMsg, reqID, data}
}

// ReplyBlockBodiesRLP creates a reply with a batch of block contents from
// an already RLP encoded format.
func (p *peer) ReplyBlockBodiesRLP(reqID uint64, bodies []srlp.RawValue) *reply {
	data, _ := srlp.EncodeToBytes(bodies)
...
func (p *peer) ReplyCode(reqID uint64, codes [][]byte) *reply {
	data, _ := srlp.EncodeToBytes(codes)
...
func (p *peer) ReplyReceiptsRLP(reqID uint64, receipts []srlp.RawValue) *reply {
	data, _ := srlp.EncodeToBytes(receipts)
...
func (p *peer) ReplyProofsV2(reqID uint64, proofs light.NodeList) *reply {
	data, _ := srlp.EncodeToBytes(proofs)
...
func (p *peer) ReplyHelperTrieProofs(reqID uint64, resp HelperTrieResps) *reply {
	data, _ := srlp.EncodeToBytes(resp)
...
func (p *peer) ReplyTxStatus(reqID uint64, stats []light.TxStatus) *reply {
	data, _ := srlp.EncodeToBytes(stats)
...
func (p *peer) SendTxs(reqID, cost uint64, txs srlp.RawValue) error {
...
type keyValueMap map[string]srlp.RawValue
...
func (l keyValueList) add(key string, val interface{}) keyValueList {
	...
	enc, err := srlp.EncodeToBytes(val)
	if err == nil {
		entry.Value = enc
	}
...
func (m keyValueMap) get(key string, val interface{}) error {
	...
	return srlp.DecodeBytes(enc, val)
}
