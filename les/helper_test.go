...
import (
	...
	"github.com/susy-go/susy-graviton/les/flowcontrol"
...
func testIndexers(db sofdb.Database, odr light.OdrBackend, iConfig *light.IndexerConfig) (*core.ChainIndexer, *core.ChainIndexer, *core.ChainIndexer) {
	chtIndexer := light.NewChtIndexer(db, odr, iConfig.ChtSize, iConfig.ChtConfirms)
	bloomIndexer := sof.NewBloomIndexer(db, iConfig.BloomSize, iConfig.BloomConfirms)
...
func newTestProtocolManager(lightSync bool, blocks int, generator func(int, *core.BlockGen), odr *LesOdr, peers *peerSet, db sofdb.Database, ulcConfig *sof.ULCConfig, testCost uint64, clock mclock.Clock) (*ProtocolManager, error) {
	var (
		svmux  = new(event.TypeMux)
	...
	if lightSync {
		...
		gchain, _ := core.GenerateChain(gspec.Config, genesis, sofash.NewFaker(), db, blocks, generator)
	...
	pm, err := NewProtocolManager(gspec.Config, indexConfig, lightSync, NetworkId, svmux, engine, peers, chain, pool, db, odr, nil, nil, make(chan struct{}), new(sync.WaitGroup), ulcConfig, func() bool { return true })
	if err != nil {
		return nil, err
	}
	if !lightSync {
...
// fails the test.
func newTestProtocolManagerMust(t *testing.T, lightSync bool, blocks int, generator func(int, *core.BlockGen), odr *LesOdr, peers *peerSet, db sofdb.Database, ulcConfig *sof.ULCConfig) *ProtocolManager {
