...
func newFreeClientPool(db sofdb.Database, freeClientCap uint64, totalLimit int, clock mclock.Clock, removePeer func(string), metricsLogger, eventLogger *csvlogger.Logger) *freeClientPool {
	pool := &freeClientPool{
		db:               db,
		clock:            clock,
...
func (f *freeClientPool) loadFromDb() {
	...
	err = srlp.DecodeBytes(enc, &storage)
	if err != nil {
...
func (f *freeClientPool) saveToDb() {
	...
	enc, err := srlp.EncodeToBytes(storage)
	if err != nil {
...
func (e *freeClientPoolEntry) EncodeRLP(w io.Writer) error {
	return srlp.Encode(w, []interface{}{e.address, uint64(e.logUsage)})
}

func (e *freeClientPoolEntry) DecodeRLP(s *srlp.Stream) error {
	var entry struct {
		Address  string
		LogUsage uint64
	}
