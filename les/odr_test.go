...
type odrTestFn func(ctx context.Context, db sofdb.Database, config *params.ChainConfig, bc *core.BlockChain, lc *light.LightChain, bhash common.Hash) []byte

func TestOdrGetBlockLes2(t *testing.T) { testOdr(t, 2, 1, true, odrGetBlock) }

func odrGetBlock(ctx context.Context, db sofdb.Database, config *params.ChainConfig, bc *core.BlockChain, lc *light.LightChain, bhash common.Hash) []byte {
...
func odrGetReceipts(ctx context.Context, db sofdb.Database, config *params.ChainConfig, bc *core.BlockChain, lc *light.LightChain, bhash common.Hash) []byte {
...
func odrAccounts(ctx context.Context, db sofdb.Database, config *params.ChainConfig, bc *core.BlockChain, lc *light.LightChain, bhash common.Hash) []byte {
...
func odrContractCall(ctx context.Context, db sofdb.Database, config *params.ChainConfig, bc *core.BlockChain, lc *light.LightChain, bhash common.Hash) []byte {
	...
	for i := 0; i < 3; i++ {
		...
		if bc != nil {
			...
			if err == nil {
				...
				context := core.NewSVMContext(msg, header, bc, nil)
				vmenv := vm.NewSVM(context, statedb, config, vm.Config{})
			...
			context := core.NewSVMContext(msg, header, lc, nil)
			vmenv := vm.NewSVM(context, state, config, vm.Config{})
			gp := new(core.GasPool).AddGas(math.MaxUint64)
			ret, _, _, _ := core.ApplyMessage(vmenv, msg, gp)
			if state.Error() == nil {
				res = append(res, ret...)
...
func odrTxStatus(ctx context.Context, db sofdb.Database, config *params.ChainConfig, bc *core.BlockChain, lc *light.LightChain, bhash common.Hash) []byte {
