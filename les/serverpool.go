...
func newServerPool(db sofdb.Database, quit chan struct{}, wg *sync.WaitGroup, trustedNodes []string) *serverPool {
	pool := &serverPool{
		db:           db,
		quit:         quit,
		wg:           wg,
...
func (pool *serverPool) saveNodes() {
	...
	enc, err := srlp.EncodeToBytes(list)
	if err == nil {
...
func (e *poolEntry) EncodeRLP(w io.Writer) error {
	return srlp.Encode(w, &poolEntryEnc{
...
func (e *poolEntry) DecodeRLP(s *srlp.Stream) error {
...
func (s *poolStats) EncodeRLP(w io.Writer) error {
	return srlp.Encode(w, []interface{}{math.Float64bits(s.sum), math.Float64bits(s.weight)})
}

func (s *poolStats) DecodeRLP(st *srlp.Stream) error {
	var stats struct {
