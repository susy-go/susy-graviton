...
func (a *announceData) sign(privKey *ecdsa.PrivateKey) {
	srlp, _ := srlp.EncodeToBytes(announceBlock{a.Hash, a.Number, a.Td})
	sig, _ := crypto.Sign(crypto.Keccak256(srlp), privKey)
...
func (a *announceData) checkSignature(id enode.ID) error {
	...
	srlp, _ := srlp.EncodeToBytes(announceBlock{a.Hash, a.Number, a.Td})
	recPubkey, err := crypto.SigToPub(crypto.Keccak256(srlp), sig)
	if err != nil {
		return err
	}
...
type proofsData [][]srlp.RawValue

type txStatus struct {
	Status core.TxStatus
	Lookup *rawdb.TxLookupEntry `srlp:"nil"`
	Error  string
}
