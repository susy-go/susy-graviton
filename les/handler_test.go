...
func testGetCHTProofs(t *testing.T, protocol int) {
	...
	srlp, _ := srlp.EncodeToBytes(header)
	...
	case 1:
		...
		trie, _ := trie.New(root, trie.NewDatabase(sofdb.NewTable(server.db, light.ChtTablePrefix)))
...
func TestGetBloombitsProofs(t *testing.T) {
	...
	for bit := 0; bit < 2048; bit++ {
		...
		trie, _ := trie.New(root, trie.NewDatabase(sofdb.NewTable(server.db, light.BloomTrieTablePrefix)))
...
func TestTransactionStatusLes2(t *testing.T) {
	...
	test := func(tx *types.Transaction, send bool, expStatus txStatus) {
		reqID++
		if send {
			enc, _ := srlp.EncodeToBytes(types.Transactions{tx})
			cost := peer.GetTxRelayCost(1, len(enc))
			sendRequest(peer.app, SendTxV2Msg, reqID, cost, srlp.RawValue(enc))
		} else {
	...
	gchain, _ := core.GenerateChain(params.TestChainConfig, chain.GetBlockByNumber(0), sofash.NewFaker(), db, 1, func(i int, block *core.BlockGen) {
		block.AddTx(tx1)
		block.AddTx(tx2)
	})
	...
	gchain, _ = core.GenerateChain(params.TestChainConfig, chain.GetBlockByNumber(0), sofash.NewFaker(), db, 2, func(i int, block *core.BlockGen) {})
	if _, err := chain.InsertChain(gchain); err != nil {
		panic(err)
	}
