...
func (alwaysDenyUI) OnApprovedTx(tx sofapi.SignTransactionResult) {
	panic("implement me")
}
...
func TestListRequest(t *testing.T) {
	...
	resp, _ := r.ApproveListing(&core.ListRequest{
		Accounts: accs,
		Meta:     core.Metadata{Remote: "remotsip", Local: "localip", Scheme: "inproc"},
	})
...
func (d *dummyUI) OnApprovedTx(tx sofapi.SignTransactionResult) {
...
func TestForwarding(t *testing.T) {
	...
	r.OnApprovedTx(sofapi.SignTransactionResult{})
...
func TestLimitWindow(t *testing.T) {
	...
	for i := 0; i < 3; i++ {
		...
		response := sofapi.SignTransactionResult{
			Tx:  dummySigned(v),
...
func (d *dontCallMe) OnApprovedTx(tx sofapi.SignTransactionResult) {
	d.t.Fatalf("Did not expect next-handler to be called")
}
