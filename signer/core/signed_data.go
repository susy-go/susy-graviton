...
func (api *SignerAPI) determineSignatureFormat(ctx context.Context, contentType string, addr common.MixedcaseAddress, data interface{}) (*SignDataRequest, bool, error) {
	var (
		req          *SignDataRequest
		useSophonV = true // Default to use V = 27 or 28, the legacy Sophon format
	)
	mediaType, _, err := mime.ParseMediaType(contentType)
	if err != nil {
		return nil, useSophonV, err
	}
	...
	case IntendedValidator.Mime:
		...
		message := []*NameValueType{
			{
				Name:  "This is a request to sign data intended for a particular validator (see SIP 191 version 0)",
				Typ:   "description",
				Value: "",
			},
			{
	...
	case ApplicationClique.Mime:
		// Clique is the Sophon PoA standard
		stringData, ok := data.(string)
		if !ok {
			return nil, useSophonV, fmt.Errorf("input for %v must be an hex-encoded string", ApplicationClique.Mime)
		}
		...
		header := &types.Header{}
		if err := srlp.DecodeBytes(cliqueData, header); err != nil {
			return nil, useSophonV, err
		}
		...
		// Get back the srlp data, encoded by us
	...
	default: // also case TextPlain.Mime:
		// Calculates an Sophon ECDSA signature for:
		// hash = keccak256("\x19${byteVersion}Sophon Signed Message:\n${message length}${message}")
		// We expect it to be a string
		if stringData, ok := data.(string); !ok {
			return nil, useSophonV, fmt.Errorf("input for text/plain must be an hex-encoded string")
		} else {
...
func cliqueHeaderHashAndRlp(header *types.Header) (hash, srlp []byte, err error) {
	if len(header.Extra) < 65 {
		err = fmt.Errorf("clique header extradata too short, %d < 65", len(header.Extra))
		return
	}
	srlp = clique.CliqueRLP(header)
	hash = clique.SealHash(header).Bytes()
	return hash, srlp, err
}

// SignTypedData signs SIP-712 conformant typed data
...
func (typedData *TypedData) Format() []*NameValueType {
	...
	nvts = append(nvts, &NameValueType{
		Name:  "SIP712Domain",
		Value: typedData.formatData("SIP712Domain", typedData.Domain.Map()),
		Typ:   "domain",
	})
	nvts = append(nvts, &NameValueType{
...
func (domain *TypedDataDomain) validate() error {
	if domain.ChainId == big.NewInt(0) {
		return errors.New("chainId must be specified according to SIP-155")
	}
