...
import (
	...
	"github.com/susy-go/susy-graviton/accounts/usbwallet"
...
type ExternalAPI interface {
	...
	SignTransaction(ctx context.Context, args SendTxArgs, methodSelector *string) (*sofapi.SignTransactionResult, error)
...
type SignerUI interface {
	...
	OnApprovedTx(tx sofapi.SignTransactionResult)
...
// SignTransaction signs the given Transaction and returns it both as json and srlp-encoded form
func (api *SignerAPI) SignTransaction(ctx context.Context, args SendTxArgs, methodSelector *string) (*sofapi.SignTransactionResult, error) {
	var (
		err    error
	...
	srlpdata, err := srlp.EncodeToBytes(signedTx)
	response := sofapi.SignTransactionResult{Raw: srlpdata, Tx: signedTx}
...
// Export returns encrypted private key associated with the given address in susyweb keystore format.
...
// in susyweb keystore format. It will decrypt the keyJSON with the given passphrase and on successful
