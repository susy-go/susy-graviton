...
func (ui *headlessUi) OnApprovedTx(tx sofapi.SignTransactionResult) {}
...
func tmpDirName(t *testing.T) string {
	d, err := ioutil.TempDir("", "sof-keystore-test")
	if err != nil {
		t.Fatal(err)
	}
...
func TestSignTx(t *testing.T) {
	var (
		list      []common.Address
		res, res2 *sofapi.SignTransactionResult
		err       error
	)
	...
	srlp.Decode(bytes.NewReader(res.Raw), parsedTx)
	...
	srlp.Decode(bytes.NewReader(res.Raw), parsedTx2)
