...
// This file contains a miner stress test based on the Sofash consensus engine.
package main
...
func main() {
	...
	// Pre-generate the sofash mining DAG so we don't race
	sofash.MakeDataset(1, filepath.Join(os.Getenv("HOME"), ".sofash"))

	// Create an Sofash network based off of the Ropsten config
	...
	for _, node := range nodes {
		...
		if err := node.Service(&sophon); err != nil {
			panic(err)
		}
		if err := sophon.StartMining(1); err != nil {
			panic(err)
		}
	}
	time.Sleep(3 * time.Second)
	...
	for {
		...
		var sophon *sof.Sophon
		if err := nodes[index%len(nodes)].Service(&sophon); err != nil {
			panic(err)
		}
		...
		if err := sophon.TxPool().AddLocal(tx); err != nil {
			panic(err)
		}
		nonces[index]++

		// Wait if we're too saturated
		if pend, _ := sophon.TxPool().Stats(); pend > 2048 {
			time.Sleep(100 * time.Millisecond)
		}
	}
}

// makeGenesis creates a custom Sofash genesis block based on some pre-defined
// faucet accounts.
func makeGenesis(faucets []*ecdsa.PrivateKey) *core.Genesis {
	...
	genesis.Config.SIP150Hash = common.Hash{}
...
func makeMiner(genesis *core.Genesis) (*node.Node, error) {
	// Define the basic configurations for the Sophon node
	...
	// Start the node and configure a full Sophon node on it
	stack, err := node.New(config)
	if err != nil {
		return nil, err
	}
	if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
		return sof.New(ctx, &sof.Config{
			Genesis:         genesis,
			...
			GPO:             sof.DefaultConfig.GPO,
			Sofash:          sof.DefaultConfig.Sofash,
			Miner: Config{
