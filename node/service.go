...
func (ctx *ServiceContext) OpenDatabase(name string, cache int, handles int) (sofdb.Database, error) {
	if ctx.config.DataDir == "" {
		return sofdb.NewMemDatabase(), nil
	}
	db, err := sofdb.NewLDBDatabase(ctx.config.ResolvePath(name), cache, handles)
	if err != nil {
		return nil, err
	}
	return db, nil
