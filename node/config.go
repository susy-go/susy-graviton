...
import (
	...
	"github.com/susy-go/susy-graviton/accounts/external"
...
type Config struct {
	...
	// used in the devp2p node identifier. The instance name of graviton is "graviton". If no
	...
	oldGravitonResourceWarning bool
}
...
func (c *Config) NodeName() string {
	name := c.name()
	// Backwards compatibility: previous versions used title-cased "Graviton", keep that.
	if name == "graviton" || name == "graviton-testnet" {
		name = "Graviton"
	}
	if c.UserIdent != "" {
		name += "/" + c.UserIdent
...
// These resources are resolved differently for "graviton" instances.
var isOldGravitonResource = map[string]bool{
	"chaindata":          true,
	"nodes":              true,
	"nodekey":            true,
...
func (c *Config) ResolvePath(path string) string {
	...
	// by graviton 1.4 are used if they exist.
	if warn, isOld := isOldGravitonResource[path]; isOld {
		oldpath := ""
		if c.name() == "graviton" {
		...
		if oldpath != "" && common.FileExist(oldpath) {
			if warn {
				c.warnOnce(&c.oldGravitonResourceWarning, "Using deprecated resource file %s, please move this file to the 'graviton' subdirectory of datadir.", oldpath)
			}
			return oldpath
		}
	}
...
func makeAccountManager(conf *Config) (*accounts.Manager, string, error) {
	...
	if keydir == "" {
		// There is no datadir.
		keydir, err = ioutil.TempDir("", "susy-graviton-keystore")
		ephemeral = keydir
	}
