...
// Whisper represents a dark communication interface through the Sophon
...
// New creates a Whisper client ready to communicate through the Sophon P2P network.
...
func (whisper *Whisper) runMessageLoop(p *Peer, rw p2p.MsgReadWriter) error {
	for {
		...
		case powRequirementCode:
			s := srlp.NewStream(packet.Payload, uint64(packet.Size))
			i, err := s.Uint()
			if err != nil {
