...
// srlpWithoutNonce returns the RLP encoded envelope contents, except the nonce.
func (e *Envelope) srlpWithoutNonce() []byte {
	res, _ := srlp.EncodeToBytes([]interface{}{e.Expiry, e.TTL, e.Topic, e.Data})
	return res
}
...
func (e *Envelope) Seal(options *MessageParams) error {
	...
	buf := make([]byte, 64)
	h := crypto.Keccak256(e.srlpWithoutNonce())
	copy(buf[:32], h)
...
func (e *Envelope) calculatePoW(diff uint32) {
	buf := make([]byte, 64)
	srlp := e.srlpWithoutNonce()
	h := crypto.Keccak256(srlp)
	copy(buf[:32], h)
...
func (e *Envelope) Hash() common.Hash {
	if (e.hash == common.Hash{}) {
		encoded, _ := srlp.EncodeToBytes(e)
...
func (e *Envelope) DecodeRLP(s *srlp.Stream) error {
	raw, err := s.Raw()
	if err != nil {
		return err
	}
	...
	// srlp.Decoder (does not implement DecodeRLP function).
	// Only public members will be encoded.
	type srlpenv Envelope
	if err := srlp.DecodeBytes(raw, (*srlpenv)(e)); err != nil {
		return err
	}
