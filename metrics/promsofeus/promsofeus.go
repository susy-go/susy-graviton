...
// Package promsofeus exposes go-metrics into a Promsofeus format.
package promsofeus
...
// Handler returns an HTTP handler which dump metrics in Promsofeus format.
func Handler(reg metrics.Registry) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		// Aggregate all the metris into a Promsofeus collector
		c := newCollector()
		...
		for _, name := range names {
			...
			default:
				log.Warn("Unknown Promsofeus metric type", "type", fmt.Sprintf("%T", i))
			}
		}
