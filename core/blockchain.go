...
// Package core implements the Sophon consensus protocol.
package core
...
type CacheConfig struct {
	...
	TrieDirtyDisabled   bool          // whether to disable trie write caching and GC altogsophy (archive node)
...
type BlockChain struct {
	...
	db     sofdb.Database // Low level persistent database to store final content in
...
// available in the database. It initialises the default Sophon Validator and
// Processor.
func NewBlockChain(db sofdb.Database, cacheConfig *CacheConfig, chainConfig *params.ChainConfig, engine consensus.Engine, vmConfig vm.Config, shouldPreserve func(block *types.Block) bool) (*BlockChain, error) {
	if cacheConfig == nil {
	...
	// the head block (sofash cache or clique voting snapshot). Might as well do
	// it in advance.
...
func (bc *BlockChain) SetHead(head uint64) error {
	...
	updateFn := func(db sofdb.KeyValueWriter, header *types.Header) {
	...
	delFn := func(db sofdb.KeyValueWriter, hash common.Hash, num uint64) {
...
// caching it if found.
func (bc *BlockChain) GetBodyRLP(hash common.Hash) srlp.RawValue {
	// Short circuit if the body's already in the cache, retrieve otherwise
	if cached, ok := bc.bodyRLPCache.Get(hash); ok {
		return cached.(srlp.RawValue)
	}
	number := bc.hc.GetBlockNumber(hash)
	if number == nil {
		return nil
...
func (bc *BlockChain) InsertReceiptChain(blockChain types.Blocks, receiptChain []types.Receipts, ancientLimit uint64) (int, error) {
	...
	writeAncient := func(blockChain types.Blocks, receiptChain []types.Receipts) (int, error) {
		...
		for i, block := range blockChain {
			...
			// Migrate all ancient blocks. This can happen if someone upgrades from Graviton
	...
	writeLive := func(blockChain types.Blocks, receiptChain []types.Receipts) (int, error) {
		...
		for i, block := range blockChain {
			...
			stats.processed++
			if batch.ValueSize() >= sofdb.IdealBatchSize {
				if err := batch.Write(); err != nil {
					return 0, err
				}
				size += batch.ValueSize()
...
func (bc *BlockChain) writeBlockWithState(block *types.Block, receipts []*types.Receipt, state *state.StateDB) (status WriteStatus, err error) {
	...
	root, err := state.Commit(bc.chainConfig.IsSIP158(block.Number()))
	if err != nil {
		return NonStatTy, err
	}
	triedb := bc.stateCache.TrieDB()
	...
	if bc.cacheConfig.TrieDirtyDisabled {
		...
		if current := block.NumberU64(); current > TriesInMemory {
			...
			if nodes > limit || imgs > 4*1024*1024 {
				triedb.Cap(limit - sofdb.IdealBatchSize)
			}
