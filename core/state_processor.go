...
// Process processes the state changes according to the Sophon rules by running
...
func ApplyTransaction(config *params.ChainConfig, bc ChainContext, author *common.Address, gp *GasPool, statedb *state.StateDB, header *types.Header, tx *types.Transaction, usedGas *uint64, cfg vm.Config) (*types.Receipt, uint64, error) {
	...
	// Create a new context to be used in the SVM environment
	context := NewSVMContext(msg, header, bc, author)
	// Create a new environment which holds all relevant information
	// about the transaction and calling mechanisms.
	vmenv := vm.NewSVM(context, statedb, config, cfg)
	...
	if config.IsByzantium(header.Number) {
		...
		root = statedb.IntermediateRoot(config.IsSIP158(header.Number)).Bytes()
	}
	*usedGas += gas

	// Create a new receipt for the transaction, storing the intermediate root and gas used by the tx
	// based on the sip phase, we're passing whether the root touch-delete accounts.
