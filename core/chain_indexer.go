...
type ChainIndexer struct {
	chainDb  sofdb.Database      // Chain database to index the data from
	indexDb  sofdb.Database      // Prefixed table-view of the db to write index metadata into
...
func NewChainIndexer(chainDb sofdb.Database, indexDb sofdb.Database, backend ChainIndexerBackend, section, confirm uint64, throttling time.Duration, kind string) *ChainIndexer {
	c := &ChainIndexer{
		chainDb:     chainDb,
		indexDb:     indexDb,
		backend:     backend,
