...
func SetupGenesisBlock(db sofdb.Database, genesis *Genesis) (*params.ChainConfig, common.Hash, error) {
	return SetupGenesisBlockWithOverride(db, genesis, nil)
}
func SetupGenesisBlockWithOverride(db sofdb.Database, genesis *Genesis, constantinopleOverride *big.Int) (*params.ChainConfig, common.Hash, error) {
	if genesis != nil && genesis.Config == nil {
		return params.AllSofashProtocolChanges, common.Hash{}, errGenesisNoConfig
	}
...
func (g *Genesis) configOrDefault(ghash common.Hash) *params.ChainConfig {
	...
	default:
		return params.AllSofashProtocolChanges
	}
}

// ToBlock creates the genesis block and writes state of a genesis specification
// to the given database (or discards it if nil).
func (g *Genesis) ToBlock(db sofdb.Database) *types.Block {
	if db == nil {
...
func (g *Genesis) Commit(db sofdb.Database) (*types.Block, error) {
	block := g.ToBlock(db)
...
// The block is committed as the canonical head block.
func (g *Genesis) MustCommit(db sofdb.Database) *types.Block {
...
func GenesisBlockForTesting(db sofdb.Database, addr common.Address, balance *big.Int) *types.Block {
	g := Genesis{Alloc: GenesisAlloc{addr: {Balance: balance}}}
	return g.MustCommit(db)
}

// DefaultGenesisBlock returns the Sophon main net genesis block.
...
// DeveloperGenesisBlock returns the 'graviton --dev' genesis block. Note, this must
// be seeded with the
...
func decodePrealloc(data string) GenesisAlloc {
	...
	if err := srlp.NewStream(strings.NewReader(data), 0).Decode(&p); err != nil {
		panic(err)
	}
