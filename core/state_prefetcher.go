...
// Prefetch processes the state changes according to the Sophon rules by running
...
func precacheTransaction(config *params.ChainConfig, bc ChainContext, author *common.Address, gaspool *GasPool, statedb *state.StateDB, header *types.Header, tx *types.Transaction, cfg vm.Config) error {
	...
	// Create the SVM and execute the transaction
	context := NewSVMContext(msg, header, bc, author)
	vm := vm.NewSVM(context, statedb, config, cfg)
