...
type Trie interface {
	...
	Prove(key []byte, fromLevel uint, proofDb sofdb.Putter) error
}
...
func NewDatabase(db sofdb.Database) Database {
...
func NewDatabaseWithCache(db sofdb.Database, cache int) Database {
...
func (m cachedTrie) Prove(key []byte, fromLevel uint, proofDb sofdb.Putter) error {
