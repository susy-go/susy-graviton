...
// stateObject represents an Sophon account which is being modified.
//
...
type stateObject struct {
	...
	addrHash common.Hash // hash of sophon address of the account
	data     Account
	db       *StateDB
...
// Account is the Sophon consensus representation of accounts.
...
func (s *stateObject) GetCommittedState(db Database, key common.Hash) common.Hash {
	...
	if len(enc) > 0 {
		_, content, _, err := srlp.Split(enc)
		if err != nil {
			s.setError(err)
		}
...
func (s *stateObject) updateTrie(db Database) Trie {
	...
	for key, value := range s.dirtyStorage {
		...
		v, _ := srlp.EncodeToBytes(bytes.TrimLeft(value[:], "\x00"))
...
func (s *stateObject) AddBalance(amount *big.Int) {
	// SIP158: We must check emptiness for the objects such that the account
