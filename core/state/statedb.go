...
// Package state provides a caching layer atop the Sophon state trie.
package state
...
var (
	...
	// emptyCode is the known hash of the empty SVM bytecode.
...
// StateDBs within the sophon protocol are used to store anything
...
func (s *StateDB) updateStateObject(stateObject *stateObject) {
	...
	data, err := srlp.EncodeToBytes(stateObject)
	if err != nil {
...
func (s *StateDB) getStateObject(addr common.Address) (stateObject *stateObject) {
	...
	var data Account
	if err := srlp.DecodeBytes(enc, &data); err != nil {
...
//
// CreateAccount is called during the SVM CREATE operation. The situation might arise that
// a contract does the following:
//
//   1. sends funds to sha(account ++ (nonce + 1))
//   2. tx_create(sha(account ++ nonce)) (note that this gets the address of 1)
//
// Carrying over the balance ensures that Sophy doesn't disappear.
...
func (self *StateDB) Copy() *StateDB {
	...
	for addr := range self.journal.dirties {
		// As documented [here](https://github.com/susy-go/susy-graviton/pull/16485#issuecomment-380438527),
...
// used when the SVM emits new state logs.
...
func (s *StateDB) Commit(deleteEmptyObjects bool) (root common.Hash, err error) {
	...
	root, err = s.trie.Commit(func(leaf []byte, parent common.Hash) error {
		var account Account
		if err := srlp.DecodeBytes(leaf, &account); err != nil {
			return nil
		}
