...
// It returns the SVM's return value, the new state and an error if it failed.
//
...
func Execute(code, input []byte, cfg *Config) ([]byte, *state.StateDB, error) {
	...
	if cfg.State == nil {
		cfg.State, _ = state.New(common.Hash{}, state.NewDatabase(sofdb.NewMemDatabase()))
	}
	var (
...
// Create executes the code using the SVM create method
...
// SVM's return value or an error if it failed.
//
