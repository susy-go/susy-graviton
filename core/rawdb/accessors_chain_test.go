...
func checkReceiptsRLP(have, want types.Receipts) error {
	...
	for i := 0; i < len(want); i++ {
		srlpHave, err := srlp.EncodeToBytes(have[i])
		if err != nil {
			return err
		}
		srlpWant, err := srlp.EncodeToBytes(want[i])
		if err != nil {
			return err
		}
		if !bytes.Equal(srlpHave, srlpWant) {
			return fmt.Errorf("receipt #%d: receipt mismatch: have %s, want %s", i, hex.EncodeToString(srlpHave), hex.EncodeToString(srlpWant))
		}
	}
	return nil
}
