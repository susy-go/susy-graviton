...
func ReadDatabaseVersion(db sofdb.KeyValueReader) *uint64 {
	var version uint64
	...
	if err := srlp.DecodeBytes(enc, &version); err != nil {
		return nil
	}
...
func WriteDatabaseVersion(db sofdb.KeyValueWriter, version uint64) {
	enc, err := srlp.EncodeToBytes(version)
	if err != nil {
...
func ReadChainConfig(db sofdb.KeyValueReader, hash common.Hash) *params.ChainConfig {
...
func WriteChainConfig(db sofdb.KeyValueWriter, hash common.Hash, cfg *params.ChainConfig) {
	if cfg == nil {
		return
	}
...
func ReadPreimage(db sofdb.KeyValueReader, hash common.Hash) []byte {
	data, _ := db.Get(preimageKey(hash))
	return data
}

// WritePreimages writes the provided set of preimages to the database.
func WritePreimages(db sofdb.KeyValueWriter, preimages map[common.Hash][]byte) {
