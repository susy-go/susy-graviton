...
import (
	...
	"github.com/susy-go/susy-graviton/sofdb/leveldb"
	"github.com/susy-go/susy-graviton/sofdb/memorydb"
...
func NewDatabase(db sofdb.KeyValueStore) sofdb.Database {
	return &nofreezedb{
		KeyValueStore: db,
	}
}
...
// storage.
func NewDatabaseWithFreezer(db sofdb.KeyValueStore, freezer string, namespace string) (sofdb.Database, error) {
...
func NewMemoryDatabase() sofdb.Database {
...
func NewMemoryDatabaseWithCap(size int) sofdb.Database {
	return NewDatabase(memorydb.NewWithCap(size))
}

// NewLevelDBDatabase creates a persistent key-value database without a freezer
// moving immutable chain segments into cold storage.
func NewLevelDBDatabase(file string, cache int, handles int, namespace string) (sofdb.Database, error) {
...
// freezer moving immutable chain segments into cold storage.
func NewLevelDBDatabaseWithFreezer(file string, cache int, handles int, freezer string, namespace string) (sofdb.Database, error) {
	susykv, err := leveldb.New(file, cache, handles, namespace)
	if err != nil {
		return nil, err
	}
	frdb, err := NewDatabaseWithFreezer(susykv, freezer, namespace)
	if err != nil {
		susykv.Close()
		return nil, err
	}
...
func InspectDatabase(db sofdb.Database) error {
	it := db.NewIterator()
	defer it.Release()
