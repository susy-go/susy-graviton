...
func newTester(t *testing.T, confOverride func(*sof.Config)) *tester {
	...
	// Create a networkless protocol stack and start an Sophon service within
	...
	sofConf := &sof.Config{
		...
		Sophybase: common.HexToAddress(testAddress),
		Sofash: sofash.Config{
			PowMode: sofash.ModeTest,
		},
	}
	if confOverride != nil {
		confOverride(sofConf)
	}
	if err = stack.Register(func(ctx *node.ServiceContext) (node.Service, error) { return sof.New(ctx, sofConf) }); err != nil {
		t.Fatalf("failed to register Sophon protocol: %v", err)
	}
