...
import (
	...
	"github.com/susy-go/susy-graviton/internal/web3ext"
...
func (c *Console) init(preload []string) error {
	...
	if _, err := c.jsre.Run("var web3 = new Web3(jsof);"); err != nil {
	...
	flatten := "var sof = web3.sof; var personal = web3.personal; "
	for api := range apis {
		if api == "web3" {
	...
	if c.prompter != nil {
		...
		// original web3 callbacks. These will be called by the jsof.* methods after
...
func (c *Console) Welcome() {
	message := "Welcome to the Graviton JavaScript console!\n\n"

	// Print some generic Graviton metadata
	if res, err := c.jsre.Run(`
		var message = "instance: " + web3.version.node + "\n";
		try {
			message += "coinbase: " + sof.coinbase + "\n";
		} catch (err) {}
		message += "at block: " + sof.blockNumber + " (" + new Date(1000 * sof.getBlock(sof.blockNumber).timestamp) + ")\n";
		try {
