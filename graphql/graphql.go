...
// Package graphql provides a GraphQL interface to Sophon node data.
package graphql
...
// Account represents an Sophon account at a particular block.
type Account struct {
	backend     *sof.SofAPIBackend
...
// Transaction represents an Sophon transaction.
...
// Block represents an Sophon block.
...
// `Log` objects.
func runFilter(ctx context.Context, be *sof.SofAPIBackend, filter *filters.Filter) ([]*Log, error) {
...
type CallData struct {
	From     *common.Address // The Sophon address the call is from.
	To       *common.Address // The Sophon address the call is to.
...
func (b *Block) Call(ctx context.Context, args struct {
	...
	result, gas, failed, err := sofapi.DoCall(ctx, b.backend, args.Data, *b.num, vm.Config{}, 5*time.Second, b.backend.RPCGasCap())
...
func (b *Block) EstimateGas(ctx context.Context, args struct {
	...
	gas, err := sofapi.DoEstimateGas(ctx, b.backend, args.Data, *b.num, b.backend.RPCGasCap())
	return gas, err
}
...
func (p *Pending) Call(ctx context.Context, args struct {
	...
	result, gas, failed, err := sofapi.DoCall(ctx, p.backend, args.Data, rpc.PendingBlockNumber, vm.Config{}, 5*time.Second, p.backend.RPCGasCap())
	status := hexutil.Uint64(1)
	if failed {
		status = 0
	}
...
func (p *Pending) EstimateGas(ctx context.Context, args struct {
	...
	return sofapi.DoEstimateGas(ctx, p.backend, args.Data, rpc.PendingBlockNumber, p.backend.RPCGasCap())
}
...
func (r *Resolver) SendRawTransaction(ctx context.Context, args struct{ Data hexutil.Bytes }) (common.Hash, error) {
	...
	if err := srlp.DecodeBytes(args.Data, tx); err != nil {
		return common.Hash{}, err
	}
	hash, err := sofapi.SubmitTransaction(ctx, r.backend, tx)
	return hash, err
}
...
func NewHandler(be *sof.SofAPIBackend) (http.Handler, error) {
	q := Resolver{be}
...
type Service struct {
	...
	backend  *sof.SofAPIBackend // The backend that queries will operate onn.
...
func NewService(backend *sof.SofAPIBackend, endpoint string, cors, vhosts []string, timeouts rpc.HTTPTimeouts) (*Service, error) {
	return &Service{
		endpoint: endpoint,
		cors:     cors,
		vhosts:   vhosts,
...
func RegisterGraphQLService(stack *node.Node, endpoint string, cors, vhosts []string, timeouts rpc.HTTPTimeouts) error {
	return stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
		var sophon *sof.Sophon
		if err := ctx.Service(&sophon); err != nil {
			return nil, err
		}
		return NewService(sophon.APIBackend, endpoint, cors, vhosts, timeouts)
	})
}
