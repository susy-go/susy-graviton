...
const schema string = `
    ...
    scalar Bytes32
    # Address is a 20 byte Sophon address, represented as 0x-prefixed hexadecimal.
    scalar Address
    ...
    # Account is an Sophon account at a particular block.
    type Account {
    ...
    # Transaction is an Sophon transaction.
    type Transaction {
    ...
    type Block {
        ...
        # Account fetches an Sophon account at the current block's state.
    ...
    type Pending {
      ...
      transactions: [Transaction!]
      # Account fetches an Sophon account for the pending state.
      account(address: Address!): Account!
    ...
    type Query {
        # Block fetches an Sophon block by number or by hash. If neither is
