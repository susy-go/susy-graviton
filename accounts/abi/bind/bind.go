...
// Package bind generates Sophon contract Go bindings.
//
// Detailed usage document and tutorial available on the susy-graviton Wiki page:
// https://github.com/susy-go/susy-graviton/wiki/Native-DApps:-Go-bindings-to-Sophon-contracts
package bind
...
func Bind(types []string, abis []string, bytecodes []string, pkg string, lang Lang) (string, error) {
	...
	for i := 0; i < len(types); i++ {
		...
		svmABI, err := abi.JSON(strings.NewReader(abis[i]))
		if err != nil {
			return "", err
		}
		...
		for _, original := range svmABI.Methods {
		...
		for _, original := range svmABI.Events {
		...
		contracts[types[i]] = &tmplContract{
			...
			Constructor: svmABI.Constructor,
			Calls:       calls,
			Transacts:   transacts,
			Events:      events,
		}
