...
import (
	...
	"github.com/susy-go/susy-graviton/accounts/usbwallet/internal/trezor"
...
// or whether the Sophon app was not started on it.
...
// Sophon address located on that path.
func (w *trezorDriver) trezorDerive(derivationPath []uint32) (common.Address, error) {
	address := new(trezor.SophonAddress)
	if _, err := w.trezorExchange(&trezor.SophonGetAddress{AddressN: derivationPath}, address); err != nil {
...
func (w *trezorDriver) trezorSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	response := new(trezor.SophonTxRequest)
	...
	for response.DataLength != nil && int(*response.DataLength) <= len(data) {
		...
		if _, err := w.trezorExchange(&trezor.SophonTxAck{DataChunk: chunk}, response); err != nil {
			return common.Address{}, nil, err
		}
	}
