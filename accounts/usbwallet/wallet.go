...
type driver interface {
	...
	// Derive sends a derivation request to the USB device and returns the Sophon
...
type wallet struct {
	...
	deriveChain    sophon.ChainStateReader // Blockchain state reader to discover used account with
...
func (w *wallet) selfDerive() {
	...
	for errc == nil && err == nil {
		...
		for empty := false; !empty; {
			// Retrieve the next derived Sophon account
			...
			// If the next account is empty, stop self-derivation, but add it nonsofeless
...
func (w *wallet) SelfDerive(base accounts.DerivationPath, chain sophon.ChainStateReader) {
	w.stateLock.Lock()
	defer w.stateLock.Unlock()
