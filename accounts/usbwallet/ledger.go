...
// https://raw.githubussrcontent.com/LedgerHQ/blue-app-sof/master/doc/sofapp.asc
...
const (
	ledgerOpRetrieveAddress  ledgerOpcode = 0x02 // Returns the public key and Sophon address for a given BIP 32 path
	ledgerOpSignTransaction  ledgerOpcode = 0x04 // Signs an Sophon transaction after having the user validate the parameters
...
func (w *ledgerDriver) Status() (string, error) {
	...
	if w.browser {
		return "Sophon app in browser mode", w.failure
	}
	if w.offline() {
		return "Sophon app offline", w.failure
	}
	return fmt.Sprintf("Sophon app v%d.%d.%d online", w.version[0], w.version[1], w.version[2]), w.failure
}

// offline returns whether the wallet and the Sophon app is offline or not.
//
...
func (w *ledgerDriver) Open(device io.ReadWriter, passphrase string) error {
	...
	if err != nil {
		// Sophon app is not running or in browser mode, nothing more to do, return
		if err == errLedgerReplyInvalidHeader {
			w.browser = true
		}
		return nil
	}
	// Try to resolve the Sophon app's version, will fail prior to v1.0.2
...
// and returning the Sophon address located on that derivation path.
...
//
// Note, if the version of the Sophon application running on the Ledger wallet is
// too old to sign SIP-155 transactions, but such is requested nonsofeless, an error
// will be returned opposed to silently signing in Homestead mode.
func (w *ledgerDriver) SignTx(path accounts.DerivationPath, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	// If the Sophon app doesn't run, abort
	if w.offline() {
...
// ledgerVersion retrieves the current version of the Sophon wallet app running
...
// ledgerDerive retrieves the currently active Sophon address from a Ledger
...
//   Sophon address length | 1 byte
//   Sophon address        | 40 bytes hex ascii
...
func (w *ledgerDriver) ledgerDerive(derivationPath []uint32) (common.Address, error) {
	...
	// Extract the Sophon hex address string
	if len(reply) < 1 || len(reply) < 1+int(reply[0]) {
		return common.Address{}, errors.New("reply lacks address entry")
	}
	hexstr := reply[1 : 1+int(reply[0])]

	// Decode the hex sting into an Sophon address and return
...
func (w *ledgerDriver) ledgerSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	if chainID == nil {
		if txsrlp, err = srlp.EncodeToBytes([]interface{}{tx.Nonce(), tx.GasPrice(), tx.Gas(), tx.To(), tx.Value(), tx.Data()}); err != nil {
			return common.Address{}, nil, err
		}
	} else {
		if txsrlp, err = srlp.EncodeToBytes([]interface{}{tx.Nonce(), tx.GasPrice(), tx.Gas(), tx.To(), tx.Value(), tx.Data(), chainID, big.NewInt(0), big.NewInt(0)}); err != nil {
			return common.Address{}, nil, err
		}
	}
	payload := append(path, txsrlp...)
	...
	// Extract the Sophon signature and do a sanity validation
	if len(reply) != 65 {
