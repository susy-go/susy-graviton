...
of the BMT hash), Using Keccak256 SHA3 hash is 32 bytes, the SVM word size to optimize for on-chain BMT verification
...
// meta: metadata to hash togsophy with BMT root for the final digest
...
func (sw *AsyncHasher) Sum(b []byte, length int, meta []byte) (s []byte) {
	...
	// hash togsophy meta and BMT root hash using the pools
