...
import (
	...
	"github.com/susy-go/susy-graviton/swarm/sctx"
...
func TestAPIResolve(t *testing.T) {
	...
	tests := []*test{
		...
		},
		{
			...
			dns:       nil,
			addr:      ensAddr,
			expectErr: errors.New(`no DNS to resolve name: "swarm.sof"`),
		},
		{
		...
		},
		{
			...
			addr:      ensAddr,
			immutable: true,
			expectErr: errors.New(`immutable address not a content hash: "swarm.sof"`),
		},
		{
			desc:      "DNS configured, ENS address, name doesn't resolve, returns error",
			dns:       doesntResolve,
			addr:      ensAddr,
			expectErr: errors.New(`DNS name not found: "swarm.sof"`),
		},
	}
	for _, x := range tests {
...
func TestMultiResolver(t *testing.T) {
	...
	sofAddr := "swarm.sof"
	sofHash := "0x2222222222222222222222222222222222222222222222222222222222222222"
	sofResolve := newTestResolveValidator(sofHash)
	...
	tests := []struct {
		...
		},
		{
			...
			r:      NewMultiResolver(MultiResolverOptionWithResolver(sofResolve, "")),
			addr:   sofAddr,
			result: sofHash,
		},
		{
			...
			r: NewMultiResolver(
				MultiResolverOptionWithResolver(sofResolve, ""),
				MultiResolverOptionWithResolver(sofResolve, ""),
			),
			addr:   sofAddr,
			result: sofHash,
		...
		},
		{
			...
			r: NewMultiResolver(
				MultiResolverOptionWithResolver(doesntResolve, ""),
				MultiResolverOptionWithResolver(sofResolve, "sof"),
			),
			addr:   sofAddr,
			result: sofHash,
		},
		{
			...
			r: NewMultiResolver(
				MultiResolverOptionWithResolver(doesntResolve, "sof"),
				MultiResolverOptionWithResolver(doesntResolve, "sof"),
				MultiResolverOptionWithResolver(sofResolve, "sof"),
			),
			addr:   sofAddr,
		...
		},
		{
			...
			addr:   sofAddr,
			result: sofHash,
			err:    errors.New(`DNS name not found: "swarm.sof"`),
		},
		{
