...
#
# The cluster contains a bootnode, a graviton node and multiple swarm nodes, with
...
# static graviton configuration
GRAVITON_IP="192.168.33.3"
GRAVITON_RPC_PORT="8545"
GRAVITON_RPC_URL="http://${GRAVITON_IP}:${GRAVITON_RPC_PORT}"
...
# start_bootnode starts a bootnode which is used to bootstrap the graviton and
# swarm nodes
start_bootnode() {
...
# start_graviton_node starts a graviton node with --datadir pointing at <base-dir>/graviton
# and a single, unlocked account with password "graviton"
start_graviton_node() {
  local dir="${base_dir}/graviton"
  mkdir -p "${dir}"

  local password="graviton"
  echo "${password}" > "${dir}/password"

  # create an account if necessary
  if [[ ! -e "${dir}/keystore" ]]; then
    info "creating graviton account"
  ...
  if [[ -z "${address}" ]]; then
    fail "failed to get graviton account address"
  fi
  ...
  local args=(
    ...
    --rpc
    --rpcaddr   "${GRAVITON_IP}"
    --rpcport   "${GRAVITON_RPC_PORT}"
    --verbosity "6"
  )

  start_node "graviton" "${GRAVITON_IP}" "$(which graviton)" ${args[@]}
}
...
create_node_network() {
  ...
  # create the vsof pair
  local vsof0="vsof${name}0"
  local vsof1="vsof${name}1"
  ip link add name "${vsof0}" type vsof peer name "${vsof1}"

  # add one end to the bridge
  ip link set dev "${vsof0}" master "${BRIDGE_NAME}"
  ip link set dev "${vsof0}" up

  # add the other end to the namespace, rename it sof0 and give it the ip
  ip link set dev "${vsof1}" netns "${name}"
  ip netns exec "${name}" ip link set dev "${vsof1}" name "sof0"
  ip netns exec "${name}" ip link set dev "sof0" up
  ip netns exec "${name}" ip address add "${ip}/24" dev "sof0"
}
...
create_account() {
  ...
  graviton --datadir "${dir}" --password /dev/stdin account new <<< "${password}"
}
