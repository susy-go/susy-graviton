...
func (b ENRAddrEntry) EncodeRLP(w io.Writer) error {
	log.Debug("in encodesrlp", "b", b, "p", fmt.Sprintf("%p", &b))
	return srlp.Encode(w, &b.data)
}

// DecodeRLP implements srlp.Decoder
func (b *ENRAddrEntry) DecodeRLP(s *srlp.Stream) error {
	byt, err := s.Bytes()
	if err != nil {
		return err
	}
	b.data = byt
	log.Debug("in decodesrlp", "b", b, "p", fmt.Sprintf("%p", &b))
	return nil
}
