...
func NewProtocolMsg(code uint64, msg interface{}) ([]byte, error) {
	...
	srlpdata, err := srlp.EncodeToBytes(msg)
	if err != nil {
		return nil, err
	}

	// TODO verify that nested structs cannot be used in srlp
	smsg := &ProtocolMsg{
		Code:    code,
		Size:    uint32(len(srlpdata)),
		Payload: srlpdata,
	}

	return srlp.EncodeToBytes(smsg)
}
...
func (prw *PssReadWriter) WriteMsg(msg p2p.Msg) error {
	...
	srlpdata := make([]byte, msg.Size)
	msg.Payload.Read(srlpdata)
	pmsg, err := srlp.EncodeToBytes(ProtocolMsg{
		Code:    msg.Code,
		Size:    msg.Size,
		Payload: srlpdata,
	})
...
func ToP2pMsg(msg []byte) (p2p.Msg, error) {
	payload := &ProtocolMsg{}
	if err := srlp.DecodeBytes(msg, payload); err != nil {
