...
`pss` is under active development, and the first implementation is yet to be merged to the Sophon main branch. Expect things to change.
...
The code tutorial [p2p programming in susy-graviton](https://github.com/nolash/susy-graviton-p2p-demo) by [@nolash](https://github.com/nolash) provides step-by-step code examples for usage of `pss` API with `susy-graviton` nodes.
...
The `pss` API is available through IPC and Websockets. There is currently no `susyweb.js` implementation, as this does not support message subscription.

For `golang` clients, please use the `rpc.Client` provided by the `susy-graviton` repository. The return values may have special types in `golang`. Please refer to `godoc` for details.
