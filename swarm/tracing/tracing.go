...
import (
	...
	"github.com/susy-go/susy-graviton/swarm/spancontext"

	opentracing "github.com/opentracing/opentracing-go"
	jaeger "github.com/uber/jaeger-client-go"
	jaegsrcfg "github.com/uber/jaeger-client-go/config"
...
const (
	...
	// StoreLabelMeta is the context value key that togsophy with StoreLabelId constitutes the retrieval key for saved spans in the span store
...
func initTracer(endpoint, svc string) (closer io.Closer) {
	...
	cfg := jaegsrcfg.Configuration{
		Sampler: &jaegsrcfg.SamplerConfig{
			Type:  jaeger.SamplerTypeConst,
			Param: 1,
		},
		Reporter: &jaegsrcfg.ReporterConfig{
			LogSpans:            true,
	...
	closer, err := cfg.InitGlobalTracer(
		svc,
		//jaegsrcfg.Logger(jLogger),
		//jaegsrcfg.Metrics(jMetricsFactory),
		//jaegsrcfg.Observer(rpcmetrics.NewObserver(jMetricsFactory, rpcmetrics.DefaultNameNormalizer)),
	)
	if err != nil {
