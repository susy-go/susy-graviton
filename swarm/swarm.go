...
import (
	...
	"github.com/susy-go/susy-graviton/swarm/storage/feed"
	...
	"github.com/susy-go/susy-graviton/contracts/chequebook"
	...
	"github.com/susy-go/susy-graviton/p2p/protocols"
	"github.com/susy-go/susy-graviton/params"
	"github.com/susy-go/susy-graviton/rpc"
	"github.com/susy-go/susy-graviton/swarm/api"
	httpapi "github.com/susy-go/susy-graviton/swarm/api/http"
	"github.com/susy-go/susy-graviton/swarm/fuse"
	"github.com/susy-go/susy-graviton/swarm/log"
	"github.com/susy-go/susy-graviton/swarm/network"
	"github.com/susy-go/susy-graviton/swarm/network/stream"
	"github.com/susy-go/susy-graviton/swarm/pss"
	...
	"github.com/susy-go/susy-graviton/swarm/swap"
	"github.com/susy-go/susy-graviton/swarm/tracing"
)
...
func NewSwarm(config *api.Config, mockStore *mock.NodeStore) (self *Swarm, err error) {
	...
	if config.SwapAPI != "" && config.SwapEnabled {
		...
		backend, err = sofclient.Dial(config.SwapAPI)
		if err != nil {
	...
	if isLegacy {
		return nil, errors.New("Legacy database format detected! Please read the migration announcement at: https://github.com/sophysphere/susy-graviton/wiki/Swarm-v0.4-local-store-migration")
	}
...
func newEnsClient(endpoint string, addr common.Address, config *api.Config, privkey *ecdsa.PrivateKey) (*ensClient, error) {
	...
	sofClient := sofclient.NewClient(client)
	...
	dns, err := ens.NewENS(transactOpts, ensRoot, sofClient)
	if err != nil {
		return nil, err
	}
...
func detectEnsAddr(client *rpc.Client) (common.Address, error) {
	...
	block, err := sofclient.NewClient(client).BlockByNumber(ctx, big.NewInt(0))
	if err != nil {
		return common.Address{}, err
	}
