...
// deocodeSIP1577ContentHash decodes a chain-stored content hash from an ENS record according to SIP-1577
...
func decodeSIP1577ContentHash(buf []byte) (storageNs, contentType, hashType, hashLength uint64, hash []byte, err error) {
	if len(buf) < 10 {
...
func extractContentHash(buf []byte) (common.Hash, error) {
	storageNs, _ /*contentType*/, _ /* hashType*/, decodedHashLength, hashBytes, err := decodeSIP1577ContentHash(buf)
	...
	//todo: for the time being we implement loose enforcement for the SIP rules until ENS manager is updated
