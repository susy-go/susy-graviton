...
func TestSIPSpecCidDecode(t *testing.T) {
	const (
		sipSpecHash = "e3010170122029f2d17be6139079dc48696d1f582a8530eb9805b561eda517e22a892c7e3f1f"
		sipHash     = "29f2d17be6139079dc48696d1f582a8530eb9805b561eda517e22a892c7e3f1f"
		dagPb       = 0x70
		sha2256     = 0x12
	)
	b, err := hex.DecodeString(sipSpecHash)
	if err != nil {
		t.Fatal(err)
	}
	hashBytes, err := hex.DecodeString(sipHash)

	if err != nil {
		t.Fatal(err)
	}

	storageNs, contentType, hashType, hashLength, decodedHashBytes, err := decodeSIP1577ContentHash(b)
...
func TestManualCidDecode(t *testing.T) {
	...
	for _, v := range []struct {
		...
		t.Run(v.name, func(t *testing.T) {
			const sipHash = "29f2d17be6139079dc48696d1f582a8530eb9805b561eda517e22a892c7e3f1f"
...
func TestManuelCidEncode(t *testing.T) {
	// call cid encode method with hash. expect byte slice returned, compare according to spec
	const sipHash = "29f2d17be6139079dc48696d1f582a8530eb9805b561eda517e22a892c7e3f1f"
	cidBytes, err := EncodeSwarmHash(common.HexToHash(sipHash))
	if err != nil {
		t.Fatal(err)
	}
