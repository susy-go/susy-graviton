...
var (
	...
	EOL = errors.New("srlp: end of list")

	// Actual Errors
	ErrExpectedString   = errors.New("srlp: expected String or Byte")
	ErrExpectedList     = errors.New("srlp: expected List")
	ErrCanonInt         = errors.New("srlp: non-canonical integer format")
	ErrCanonSize        = errors.New("srlp: non-canonical size information")
	ErrElemTooLarge     = errors.New("srlp: element is larger than containing list")
	ErrValueTooLarge    = errors.New("srlp: value size exceeds available input length")
	ErrMoreThanOneValue = errors.New("srlp: input contains more than one value")

	// internal errors
	errNotInList     = errors.New("srlp: call of ListEnd outside of any list")
	errNotAtEOL      = errors.New("srlp: call of ListEnd not positioned at EOL")
	errUintOverflow  = errors.New("srlp: uint overflow")
	errNoPointer     = errors.New("srlp: interface given to Decode must be a pointer")
	errDecodeIntoNil = errors.New("srlp: pointer given to Decode must not be nil")
...
func (err *decodeError) Error() string {
	...
	return fmt.Sprintf("srlp: %s for %v%s", err.msg, err.typ, ctx)
}
...
func makeDecoder(typ reflect.Type, tags tags) (dec decoder, err error) {
	...
	default:
		return nil, fmt.Errorf("srlp: type %v is not RLP-serializable", typ)
	}
}
...
func decodeInterface(s *Stream, val reflect.Value) error {
	if val.Type().NumMethod() != 0 {
		return fmt.Errorf("srlp: type %v is not RLP-serializable", val.Type())
	}
	kind, _, err := s.Kind()
	if err != nil {
		return err
...
func (s *Stream) Bool() (bool, error) {
	...
	case 1:
		return true, nil
	default:
		return false, fmt.Errorf("srlp: invalid boolean value: %d", num)
	}
}
