...
// See https://github.com/susy-go/SIPs/issues/176 for the test format specification.
...
func (t *StateTest) Run(subtest StateSubtest, vmconfig vm.Config) (*state.StateDB, error) {
	...
	statedb := MakePreState(sofdb.NewMemDatabase(), t.json.Pre)
	...
	context := core.NewSVMContext(msg, block.Header(), nil, &t.json.Env.Coinbase)
	context.GetHash = vmTestBlockHash
	svm := vm.NewSVM(context, statedb, config, vmconfig)

	gaspool := new(core.GasPool)
	gaspool.AddGas(block.GasLimit())
	snapshot := statedb.Snapshot()
	if _, _, _, err := core.ApplyMessage(svm, msg, gaspool); err != nil {
		statedb.RevertToSnapshot(snapshot)
	}
	// Commit block
	statedb.Commit(config.IsSIP158(block.Number()))
	...
	root := statedb.IntermediateRoot(config.IsSIP158(block.Number()))
	// N.B: We need to do this in a two-step process, because the first Commit takes care
	// of suicides, and we need to touch the coinbase _after_ it has potentially suicided.
	if root != common.Hash(post.Root) {
		return statedb, fmt.Errorf("post state root mismatch: got %x, want %x", root, post.Root)
	}
	if logs := srlpHash(statedb.Logs()); logs != common.Hash(post.Logs) {
...
func MakePreState(db sofdb.Database, accounts core.GenesisAlloc) *state.StateDB {
...
func (tx *stTransaction) toMessage(ps stPostState) (core.Message, error) {
	...
	// Value, Data hex encoding is messy: https://github.com/susy-go/tests/issues/203
	value := new(big.Int)
	if valueHex != "0x" {
