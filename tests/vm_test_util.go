...
// VMTest checks SVM execution without block or transaction context.
// See https://github.com/susy-go/tests/wiki/VM-Tests for the test format specification.
type VMTest struct {
	json vmJSON
}
...
func (t *VMTest) Run(vmconfig vm.Config) error {
	...
	// }
	if logs := srlpHash(statedb.Logs()); logs != common.Hash(t.json.Logs) {
...
func (t *VMTest) exec(statedb *state.StateDB, vmconfig vm.Config) ([]byte, uint64, error) {
	svm := t.newSVM(statedb, vmconfig)
	e := t.json.Exec
	return svm.Call(vm.AccountRef(e.Caller), e.Address, e.Data, e.GasLimit, e.Value)
}

func (t *VMTest) newSVM(statedb *state.StateDB, vmconfig vm.Config) *vm.SVM {
	initialCall := true
	...
	return vm.NewSVM(context, statedb, params.MainnetChainConfig, vmconfig)
}
