...
func TestState(t *testing.T) {
	...
	st.skipLoad(`^stTransactionTest/zeroSigTransa[^/]*\.json`) // SIP-86 is not supported yet
	// Expected failures:
...
var testVMConfig = func() vm.Config {
	vmconfig := vm.Config{}
	flag.StringVar(&vmconfig.SVMInterpreter, utils.SVMInterpreterFlag.Name, utils.SVMInterpreterFlag.Value, utils.SVMInterpreterFlag.Usage)
...
func withTrace(t *testing.T, gasLimit uint64, test func(vm.Config) error) {
	...
	if gasLimit > traceErrorLimit {
		t.Log("gas limit too high for SVM trace")
		return
	}
	...
	w.Flush()
	if buf.Len() == 0 {
		t.Log("no SVM operation logs generated")
	} else {
		t.Log("SVM operation log:\n" + buf.String())
	}
	//t.Logf("SVM output: 0x%x", tracer.Output())
	//t.Logf("SVM error: %v", tracer.Error())
}
