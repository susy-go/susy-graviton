...
type RLPTest struct {
	...
	// calls to srlp.Stream. The test also verifies that encoding
...
func checkDecodeFromJSON(s *srlp.Stream, exp interface{}) error {
	switch exp := exp.(type) {
	case uint64:
		i, err := s.Uint()
		if err != nil {
